...
const X64_PROVENANCE_SHA: &str = "233a40905c350398edeb1eacad7ef43b68a8b84f0cf520201c27071dc3a70124";
const ARM64_PROVENANCE_SHA: &str =
...
if [ "$1" = precache ] && [ "$2" = --help ]; then
  printf '%s\n' 'Populate the Flutter tool cache.' '    --linux    Precache Linux artifacts.'
  exit 0
fi
...
fn embedded_catalog_keeps_storage_and_pub_independent_and_content_complete() {
    ...
    for candidate in complete_storage {
        ...
        assert_eq!(
            ...
            Some(X64_PROVENANCE_SHA)
        );
        assert_eq!(candidate.probes[2].method, HttpMethod::Head);
        assert_eq!(
            candidate.probes[3].sha256.as_deref(),
            Some(ARM64_PROVENANCE_SHA)
        );
        assert_eq!(candidate.probes[4].method, HttpMethod::Head);
    }
    assert_eq!(
        storage
            .iter()
