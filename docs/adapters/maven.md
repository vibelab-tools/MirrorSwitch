...
`x86_64` and `arm64`. Maven 4 remains outside this adapter until its project-settings and settings
2.0 precedence model receives separate acceptance coverage. Detection records Maven and launcher
JDK versions, Maven home, global and user settings, project POM repositories, plugin repositories,
and project or environment settings overrides.
