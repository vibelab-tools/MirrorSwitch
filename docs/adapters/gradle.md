...
The Gradle adapter supports the reviewed Gradle `7.6` through `9.x` model on Linux `x86_64` and
`arm64`. It detects the Gradle executable, an executable Wrapper, Wrapper distribution version and
checksum, launcher JVM, user and installation init scripts, and project settings/build repository
definitions.
