...
| Nix | `x86_64-darwin`, `aarch64-darwin` | User or daemon substituters plus trusted keys | TUNA/NJU candidate; USTC/SJTUG partial | Darwin `.narinfo`, NAR hash/signature and `nix path-info` | [#97](https://github.com/vibelab-tools/MirrorSwitch/issues/97) |
| CocoaPods | x86_64, arm64 | User Specs repository; project Podfile only when explicit | Huawei/TUNA/NJU partial legacy data; no complete modern CDN chain yet | repo/CDN metadata, podspec, declared source and `pod install` fixture | [#98](https://github.com/vibelab-tools/MirrorSwitch/issues/98) |
...
| Scoop | Windows 10/11 x64; arm64 per manifest | User bucket Git; global installation is explicit/elevated | NJU bucket Git only, installer URLs not mirrored | refs, manifest architecture branch, URL and hash, native install fixture | [#100](https://github.com/vibelab-tools/MirrorSwitch/issues/100) |
| MSYS2 | Windows 10 1809+/11 x64; Windows 11 arm64 preliminary | Installation-local signed Pacman mirrorlists | All six have current repository evidence; architecture/repo content differs | signed DB, representative package, `pacman -Syy`/query | [#102](https://github.com/vibelab-tools/MirrorSwitch/issues/102) |
| Cygwin | Windows 10/11 x86_64 only | `setup-x86_64` site and installation state | Huawei/TUNA candidate; Alibaba/NJU unconfirmed by current official list | `setup.xz`, signature/checksum and representative archive | [#104](https://github.com/vibelab-tools/MirrorSwitch/issues/104) |
...
| --- | --- | --- | --- | --- |
| pip, uv, PDM, Poetry, Conda | Planned on x86_64/arm64 | Planned on x64/arm64 | User; project/site explicit | PyPI/Conda metadata and platform subdirs; client precedence and virtual environment paths are native. |
