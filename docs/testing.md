...
`unit` enforces that every catalog tool marked `supported` has an adapter boundary test, then
runs the complete Rust suite serially. `language` runs representative Python, Node.js, JVM, Go,
and Rust adapters. `distro` pairs the relevant public adapter boundary test with an isolated
container that asks the distribution's real package manager to parse a fixture. `docker` runs
Debian, Ubuntu, Fedora, Rocky Linux, AlmaLinux, CentOS Stream, Arch Linux, openSUSE, and Alpine.
