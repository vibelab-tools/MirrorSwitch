...
const MANAGED_MARKER: &str = "# Managed by MirrorSwitch: Elastic Stack package repository v1";
const DEFAULT_KEYRING: &str = "/usr/share/keyrings/elastic-keyring.gpg";
const CHANNEL: &str = "9.x";
...
pub struct ElasticStackAdapter;

impl Adapter for ElasticStackAdapter {
    ...
    fn detect(
            ...
            Some(version) => {
                    ...
                    .is_some_and(|configured| configured != channel)
                {
                    return Err(AdapterError::Unsupported(format!(
                        "installed Elastic Stack {version} does not match configured channel {}",
        ...
        if channel != CHANNEL {
            return Err(AdapterError::Unsupported(format!(
                "Elastic Stack channel {channel} is outside reviewed 9.x coverage"
            )));
        }
        let platform = platform(context, manager)?;
        if manager == Manager::Apt && context.architecture == Architecture::Arm64 {
            return Err(AdapterError::Unsupported(
                "Elastic Stack 9.x APT has no usable arm64 package index".into(),
            ));
        }
        ...
        if configured > 1 {
            return Err(AdapterError::InvalidConfiguration(
                "multiple Elastic Stack Community server repositories are active".into(),
            ));
        }
        ...
        Ok(Some(DetectedTool {
            ...
            evidence: vec![
                ...
                format!("target Elastic Stack channel is {channel}"),
                format!(
                    "installed Elastic Stack version is {}",
                    installed.as_deref().unwrap_or("not detected")
                ),
                format!("configured Elastic Stack Community server repositories: {configured}"),
                ...
                "Elastic products, majors, private repositories, and service data are separate surfaces".into(),
            ],
        }))
    }
    ...
    fn read_current(
        ...
        require_linux(context)?;
        require_scope(scope)?;
        if detected.tool_id != "elasticstack" {
            return Err(AdapterError::InvalidConfiguration(
                "Elastic Stack package read received another tool's detection result".into(),
            ));
        }
        let manager = manager(runtime)?;
        ...
        let channel = match installed.as_deref() {
            ...
            None => configured_channel.clone().ok_or_else(|| {
                AdapterError::Unsupported(
                    "Elastic Stack channel cannot be inferred from the binary or repository".into(),
                )
            })?,
        };
        if channel != CHANNEL {
        ...
        {
            return Err(AdapterError::Unsupported(
                "installed Elastic Stack version and configured server channel differ".into(),
            ));
        }
        let platform = platform(context, manager)?;
        if manager == Manager::Apt && context.architecture == Architecture::Arm64 {
        ...
        if detected.version.as_deref() != Some(effective_version) {
            return Err(AdapterError::Conflict(
                "Elastic Stack package target version changed after detection".into(),
            ));
        }
        ...
        for document in observed {
                ...
                match manager {
                    Manager::Apt => {
                        let keyring = source.signed_by.as_deref().ok_or_else(|| {
                            AdapterError::Unsupported(
                                "modern Elastic Stack APT source has no explicit Signed-By keyring"
                                    .into(),
                            )
                        })?;
            ...
            documents.push(ConfigurationDocument {
                ...
                format: if document.path == target {
                    "elasticstack-package-target".into()
                } else {
                    "elasticstack-package-read-only".into()
                },
    ...
    fn plan(
        ...
        let target = find_document(current, "elasticstack-package-target")?;
        let rendered = if target.contents.is_empty() {
            render_new(manager, &mapped)
        } else {
            rewrite_existing(
                manager,
        ...
        let changes = if target.contents == rendered {
            ...
            vec![PlannedFileChange {
                ...
                new_mode: None,
                summary: format!(
                    "map only the Elastic Stack {channel} {} package channel while preserving GPG, pinning, credentials, and unrelated repositories",
                    manager.name()
                ),
            }]
        };
        ...
        let result = (|| {
            ...
            if !receipt.changed_targets.contains(&physical) {
                return Err(AdapterError::Verification(
                    "Elastic Stack package transaction contains no known target".into(),
                ));
            }
            let selected = documents
                ...
                .ok_or_else(|| {
                    AdapterError::Verification(
                        "Elastic Stack package repository is not active after apply".into(),
                    )
                })?;
            ...
            if !["elasticsearch", "kibana", "logstash", "filebeat"]
                .iter()
                .all(|package| output.contains(package))
                || !output.contains("9.")
            {
                return Err(AdapterError::Verification(
                    "system package manager did not expose all reviewed Elastic Stack 9.x packages"
                        .into(),
                ));
            }
            Ok(VerificationResult {
                valid: true,
                summary: format!(
                    "{} refreshed and queried Elastic Stack {} through {}",
                    manager.name(),
                    selected.channel,
                    selected.uri
                ),
    ...
    fn restore(
        ...
        Ok(RestoreResult {
            ...
            summary: format!(
                "restored {} Elastic Stack package repository files from {}",
    ...
    fn from_name(value: &str) -> Result<Self, AdapterError> {
        match value {
            ...
            _ => Err(AdapterError::InvalidConfiguration(
                "unknown Elastic Stack package manager".into(),
            )),
        }
    }
}
...
fn require_linux(context: &SystemContext) -> Result<(), AdapterError> {
    if context.os != OperatingSystem::Linux {
        return Err(AdapterError::Unsupported(
            "Elastic Stack package adapter supports Linux only".into(),
        ));
    }
    Ok(())
}
...
fn require_scope(scope: ConfigurationScope) -> Result<(), AdapterError> {
    if scope != ConfigurationScope::System {
        return Err(AdapterError::Unsupported(
            "Elastic Stack package repositories require system scope".into(),
        ));
    }
    Ok(())
}
...
fn require_current(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    if current.tool_id != "elasticstack" || current.scope != ConfigurationScope::System {
        return Err(AdapterError::InvalidConfiguration(
            "Elastic Stack package operation received another tool or scope".into(),
        ));
    }
    Ok(())
}
...
fn manager(runtime: &dyn Runtime) -> Result<Manager, AdapterError> {
    if runtime.command_exists("apt-get") && runtime.command_exists("apt-cache") {
        ...
        Err(AdapterError::Unsupported(
            "Elastic Stack packages require apt-get/apt-cache or dnf".into(),
        ))
    }
}
...
fn read_documents(
    ...
    let (main, directory, target, extensions) = match manager {
        Manager::Apt => (
            ...
            PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-elasticstack.sources"),
            &["list", "sources"][..],
        ),
        Manager::Rpm => (
            None,
            PathBuf::from("/etc/yum.repos.d"),
            PathBuf::from("/etc/yum.repos.d/mirrorswitch-elasticstack.repo"),
            &["repo"][..],
        ),
    };
...
fn parse_apt_repository(text: &str, path: &Path) -> Result<Option<RepositorySource>, AdapterError> {
    ...
    for stanza in text.split("\n\n") {
        ...
        for line in stanza.lines() {
            ...
            if let Some(value) = active.strip_prefix("URIs:") {
                for uri in value
                    .split_whitespace()
                    .filter(|uri| is_elasticstack_apt_uri(uri))
                {
                    if deb822_uri
                        ...
                        .is_some()
                    {
                        return Err(AdapterError::InvalidConfiguration(format!(
                            "multiple Elastic Stack APT URIs exist in {}",
                            path.display()
                        )));
                    }
                }
            ...
            for (index, value) in fields.iter().enumerate() {
                if !is_elasticstack_apt_uri(value) {
                    continue;
                }
                let suite = fields.get(index + 1).ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "Elastic Stack APT source in {} has no suite",
                        path.display()
                    ))
                })?;
                matches.push((
        ...
        if let Some(uri) = deb822_uri {
            matches.push((
                ...
                deb822_suite.ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "Elastic Stack deb822 source in {} has no suite",
                        path.display()
                    ))
                })?,
                deb822_components,
    ...
    matches
        ...
        .map(|(uri, suite, components, signed_by)| {
            if suite != "stable" {
                return Err(AdapterError::InvalidConfiguration(format!(
                    "Elastic Stack APT source in {} is not stable",
                    path.display()
                )));
            }
            Ok(RepositorySource {
                uri: uri.clone(),
                channel: apt_channel(uri, components, path)?,
                family: "apt".into(),
                release: "stable".into(),
...
fn apt_channel(uri: &str, components: &[String], path: &Path) -> Result<String, AdapterError> {
    if components != ["main"] {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Elastic Stack APT source in {} must use the main component",
            path.display()
        )));
    }
    uri.split_once("/packages/")
        .or_else(|| uri.split_once("/elasticstack/"))
        .and_then(|(_, value)| value.split('/').next())
        .filter(|channel| channel.ends_with(".x"))
        .map(str::to_owned)
        .ok_or_else(|| AdapterError::InvalidConfiguration("Elastic APT channel is invalid".into()))
}
...
fn parse_rpm_repository(text: &str, path: &Path) -> Result<Option<RepositorySource>, AdapterError> {
    ...
    let matches = values
        ...
        .filter_map(|(repo_id, values)| {
            ...
            (values.get("enabled").is_none_or(|value| value != "0") && is_elasticstack_rpm_uri(uri))
                .then(|| (repo_id.clone(), uri.trim_end_matches('/').to_owned()))
        })
        .collect::<Vec<_>>();
    if matches.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "multiple Elastic Stack Community server RPM repositories exist in {}",
            path.display()
        )));
    }
    matches
        ...
        .map(|(repo_id, uri)| {
            ...
            let (channel, release) = rpm_identity(uri).ok_or_else(|| {
                AdapterError::InvalidConfiguration(format!(
                    "Elastic Stack RPM URI in {} has an unknown channel or release",
                    path.display()
                ))
            })?;
            Ok(RepositorySource {
...
fn is_elasticstack_apt_uri(value: &str) -> bool {
    [
        "https://artifacts.elastic.co/packages/9.x/apt",
        "https://mirrors.aliyun.com/elasticstack/9.x/apt",
        "https://mirrors.nju.edu.cn/elasticstack/9.x/apt",
        "https://mirrors.tuna.tsinghua.edu.cn/elasticstack/9.x/apt",
    ]
    .contains(&value.trim_end_matches('/'))
}

fn is_elasticstack_rpm_uri(value: &str) -> bool {
    let value = value.trim_end_matches('/');
    (value.starts_with("https://artifacts.elastic.co/packages/")
        || ACTIONABLE_ENDPOINTS
            .iter()
            .any(|(_, endpoint)| value.starts_with(&format!("{endpoint}/9.x/yum"))))
        && value.ends_with("/9.x/yum")
}

fn rpm_identity(value: &str) -> Option<(&'static str, &'static str)> {
    value.ends_with("/9.x/yum").then_some((CHANNEL, "stable"))
}
...
fn unique_configured_channel(
    ...
    if values.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(
            "configured Elastic Stack repositories cross product channels".into(),
        ));
    }
...
fn choose_target(manager: Manager, documents: &[ObservedDocument]) -> PathBuf {
    documents
        ...
        .unwrap_or_else(|| match manager {
            ...
            Manager::Rpm => PathBuf::from("/etc/yum.repos.d/mirrorswitch-elasticstack.repo"),
        })
}

fn is_managed_path(path: &Path) -> bool {
    path.file_name().is_some_and(|name| {
        name == "mirrorswitch-elasticstack.sources" || name == "mirrorswitch-elasticstack.repo"
    })
}

fn installed_version(runtime: &dyn Runtime) -> Result<Option<String>, AdapterError> {
    if !runtime.command_exists("elasticsearch") {
        return Ok(None);
    }
    let output = runtime.run("elasticsearch", &["--version".into()])?;
    if !output.status.success() {
        return Ok(None);
    }
    let output = String::from_utf8(output.stdout)
        .map_err(|_| AdapterError::Runtime("elasticsearch --version is not UTF-8".into()))?;
    Ok(output
        .split_whitespace()
...
fn channel_from_version(value: &str) -> Result<&'static str, AdapterError> {
    match version_pair(value) {
        ...
        Some((major, minor)) => Err(AdapterError::Unsupported(format!(
            "installed Elastic Stack {major}.{minor} is outside reviewed 9.x coverage"
        ))),
        None => Err(AdapterError::Unsupported(format!(
            "installed Elastic Stack version {value} is invalid"
        ))),
    }
}
...
fn platform(context: &SystemContext, manager: Manager) -> Result<Platform, AdapterError> {
    let distribution = context.distribution.as_ref().ok_or_else(|| {
        AdapterError::Unsupported(
            "Elastic Stack packages require a detected Linux distribution".into(),
        )
    })?;
    ...
    match manager {
        Manager::Apt => {
            if !matches!(distribution.id.as_str(), "debian" | "ubuntu") {
                return Err(AdapterError::Unsupported(format!(
                    "Elastic Stack 9.x APT coverage excludes {}",
                    distribution.id
                )));
            }
            Ok(Platform {
        ...
        Manager::Rpm => {
            ...
            {
                return Err(AdapterError::Unsupported(format!(
                    "Elastic Stack 9.x RPM coverage is EL9 only, not {} {}",
...
fn mapped_uri(
    ...
    match (manager, provider) {
        (Manager::Apt, "aliyun" | "nju" | "tuna") if platform.architecture == "amd64" => {
            Ok(format!("{endpoint}/9.x/apt"))
        }
        (Manager::Apt, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed current Elastic Stack APT repository".into(),
        )),
        (Manager::Rpm, "aliyun" | "nju" | "tuna") => Ok(format!("{endpoint}/9.x/yum")),
        (Manager::Rpm, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed Elastic Stack RPM repository".into(),
        )),
    }
}

fn render_new(manager: Manager, mapped_uri: &str) -> String {
    match manager {
        Manager::Apt => format!(
            "{MANAGED_MARKER}\nTypes: deb\nURIs: {mapped_uri}\nSuites: stable\nComponents: main\nArchitectures: amd64\nSigned-By: {DEFAULT_KEYRING}\n"
        ),
        Manager::Rpm => format!(
            "{MANAGED_MARKER}\n[mirrorswitch-elasticstack]\nname=Elastic Stack 9.x\nbaseurl={mapped_uri}\nenabled=1\ngpgcheck=1\ngpgkey=https://artifacts.elastic.co/GPG-KEY-elasticsearch\n"
        ),
    }
}
...
fn rewrite_existing(
    ...
    let source = parse_repository(manager, text, path)?.ok_or_else(|| {
        AdapterError::InvalidConfiguration("selected Elastic Stack repository disappeared".into())
    })?;
    if source.channel != channel {
        return Err(AdapterError::Unsupported(
            "Elastic Stack repository channel changed during planning".into(),
        ));
    }
...
fn replace_active_once(
    ...
    if ranges.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Elastic Stack repository URI in {} is missing or duplicated",
            path.display()
        )));
    }
...
fn selected_endpoint(selections: &[MirrorSelection]) -> Result<(String, String), AdapterError> {
    if selections.len() != 1
        || selections[0].tool_id != "elasticstack"
        || selections[0].upstream_id != UPSTREAM
    {
        return Err(AdapterError::InvalidConfiguration(
            "Elastic Stack packages require exactly one repository selection".into(),
        ));
    }
    ...
    let role_url = |role| -> Result<String, AdapterError> {
        ...
        if endpoints.len() != 1 {
            return Err(AdapterError::InvalidConfiguration(format!(
                "Elastic Stack package selection needs one HTTPS {role:?} endpoint"
            )));
        }
        normalized_endpoint(&endpoints[0].url).ok_or_else(|| {
            AdapterError::InvalidConfiguration("Elastic Stack package endpoint is unsafe".into())
        })
    };
    let index = role_url(EndpointRole::Index)?;
    if role_url(EndpointRole::Metadata)? != index || role_url(EndpointRole::Packages)? != index {
        return Err(AdapterError::InvalidConfiguration(
            "Elastic Stack package index, metadata, and package endpoints must match".into(),
        ));
    }
    ...
    {
        return Err(AdapterError::InvalidConfiguration(
            "Elastic Stack package provider and endpoint are not reviewed".into(),
        ));
    }
...
fn validate_policy(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    for source in &current.sources {
        match metadata(source, "kind")? {
            "multiple-modern-repositories" => {
                return Err(AdapterError::InvalidConfiguration(
                    "multiple modern Elastic Stack package repositories are active".into(),
                ));
            }
            "coverage-mismatch" => {
                return Err(AdapterError::Unsupported(
                    "Elastic Stack repository distribution, release, or channel does not match the host"
                        .into(),
                ));
            }
            "keyring-missing" => {
                return Err(AdapterError::Unsupported(
                    "Elastic Stack APT keyring is missing; repository security cannot be preserved"
                        .into(),
                ));
            }
            "gpgcheck-disabled" => {
                return Err(AdapterError::Unsupported(
                    "Elastic Stack RPM repository has gpgcheck disabled".into(),
                ));
            }
            "gpgkey-missing" => {
                return Err(AdapterError::Unsupported(
                    "Elastic Stack RPM repository has no GPG key configured".into(),
                ));
            }
            "managed-target-conflict" => {
                return Err(AdapterError::Unsupported(
                    "Elastic Stack package target belongs to another MirrorSwitch format".into(),
                ));
            }
            _ => {}
        }
...
fn verify_apt(runtime: &dyn Runtime, target: &Path) -> Result<String, AdapterError> {
    ...
    query.extend(
        ["policy", "elasticsearch", "kibana", "logstash", "filebeat"]
            .into_iter()
            .map(str::to_owned),
    );
    command_output(runtime.run("apt-cache", &query)?, "Elastic Stack APT query")
}
...
fn verify_rpm(
    ...
    let repo_id = repo_id.unwrap_or("mirrorswitch-elasticstack");
    ...
    command_output(runtime.run("dnf", &refresh)?, "Elastic Stack RPM refresh")?;
    ...
    command_output(runtime.run("dnf", &query)?, "Elastic Stack RPM query")
}
...
fn find_document<'a>(
    ...
    if matches.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Elastic Stack package state must contain one {format} document"
        )));
    }
    Ok(matches[0])
}
...
fn current_value(current: &CurrentConfiguration, kind: &str) -> Result<String, AdapterError> {
    ...
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Elastic Stack package state has ambiguous {kind}"
        )));
    }
    Ok(values[0].clone())
}
...
fn snapshot_source(kind: &str, value: &str) -> ConfiguredSource {
    ConfiguredSource {
        upstream_id: None,
        url: format!("elasticstack-package-snapshot:{value}"),
        enabled: true,
        metadata: BTreeMap::from([
...
fn metadata<'a>(source: &'a ConfiguredSource, key: &str) -> Result<&'a str, AdapterError> {
    let values = source.metadata.get(key).ok_or_else(|| {
        AdapterError::InvalidConfiguration(format!(
            "Elastic Stack package source is missing {key} metadata"
        ))
    })?;
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Elastic Stack package source has ambiguous {key} metadata"
        )));
    }
    Ok(&values[0])
}
...
fn validate_path(path: &Path, kind: &str) -> Result<(), AdapterError> {
    ...
    {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Elastic Stack package {kind} path {} is unsafe",
            path.display()
        )));
    }
    Ok(())
...
fn utf8<'a>(path: &Path, contents: &'a [u8]) -> Result<&'a str, AdapterError> {
    std::str::from_utf8(contents).map_err(|_| {
        AdapterError::InvalidConfiguration(format!(
            "Elastic Stack package configuration {} is not UTF-8",
            path.display()
        ))
    })
}
