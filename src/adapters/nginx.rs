...
const MANAGED_MARKER: &str = "# Managed by MirrorSwitch: Nginx package repository v1";
const DEFAULT_KEYRING: &str = "/usr/share/keyrings/nginx-archive-keyring.gpg";
const ACTIONABLE_ENDPOINTS: &[(&str, &str)] = &[("nju", "https://mirrors.nju.edu.cn/nginx")];
...
impl Adapter for NginxAdapter {
    ...
    fn detect(
            ...
            Some(version) => {
                    ...
                    .is_some_and(|configured| configured != channel)
                {
                    return Err(AdapterError::Unsupported(format!(
                        "installed Nginx {version} does not match configured channel {}",
        ...
        if !matches!(channel.as_str(), "stable" | "mainline") {
            return Err(AdapterError::Unsupported(format!(
                "Nginx channel {channel} is outside reviewed stable LTS coverage"
            )));
        }
        ...
        if configured > 1 {
            return Err(AdapterError::InvalidConfiguration(
                "multiple Nginx Community server repositories are active".into(),
            ));
        }
        ...
        Ok(Some(DetectedTool {
            ...
            evidence: vec![
                ...
                format!("target Nginx channel is {channel}"),
                format!(
                    "installed Nginx version is {}",
                    installed.as_deref().unwrap_or("not detected")
                ),
                format!("configured Nginx Community server repositories: {configured}"),
                ...
                "Nginx majors, MySQL repositories, private repositories, and service data are separate surfaces".into(),
            ],
        }))
    }
    ...
    fn read_current(
        ...
        require_linux(context)?;
        require_scope(scope)?;
        if detected.tool_id != "nginx" {
            return Err(AdapterError::InvalidConfiguration(
                "Nginx package read received another tool's detection result".into(),
            ));
        }
        let manager = manager(runtime)?;
        ...
        let channel = match installed.as_deref() {
            ...
            None => configured_channel.clone().ok_or_else(|| {
                AdapterError::Unsupported(
                    "Nginx channel cannot be inferred from the binary or repository".into(),
                )
            })?,
        };
        if !matches!(channel.as_str(), "stable" | "mainline") {
            ...
            .is_some_and(|configured| configured != channel)
        {
            return Err(AdapterError::Unsupported(
                "installed Nginx version and configured server channel differ".into(),
            ));
        }
        let platform = platform(context, manager)?;
        ...
        if detected.version.as_deref() != Some(effective_version) {
            return Err(AdapterError::Conflict(
                "Nginx package target version changed after detection".into(),
            ));
        }
        ...
        for document in observed {
                ...
                match manager {
                    Manager::Apt => {
                        let keyring = source.signed_by.as_deref().ok_or_else(|| {
                            AdapterError::Unsupported(
                                "modern Nginx APT source has no explicit Signed-By keyring".into(),
                            )
                        })?;
            ...
            documents.push(ConfigurationDocument {
                ...
                format: if document.path == target {
                    "nginx-package-target".into()
                } else {
                    "nginx-package-read-only".into()
                },
    ...
    fn plan(
        ...
        let mapped = mapped_uri(manager, &provider, &endpoint, &platform, &channel)?;
        let target = find_document(current, "nginx-package-target")?;
        let rendered = if target.contents.is_empty() {
            render_new(manager, &mapped, &platform, &channel)
        } else {
            rewrite_existing(
                manager,
        ...
        let changes = if target.contents == rendered {
            ...
            vec![PlannedFileChange {
                ...
                new_mode: None,
                summary: format!(
                    "map only the Nginx {channel} {} package channel while preserving GPG, pinning, credentials, and unrelated repositories",
                    manager.name()
                ),
            }]
        };
        ...
        let result = (|| {
            ...
            if !receipt.changed_targets.contains(&physical) {
                return Err(AdapterError::Verification(
                    "Nginx package transaction contains no known target".into(),
                ));
            }
            let selected = documents
                ...
                .ok_or_else(|| {
                    AdapterError::Verification(
                        "Nginx package repository is not active after apply".into(),
                    )
                })?;
            ...
            if !normalized.contains("nginx") || !normalized.contains("1.3") {
                return Err(AdapterError::Verification(
                    "system package manager did not expose the selected Nginx 1.30/1.31 channel"
                        .into(),
                ));
            }
            Ok(VerificationResult {
                valid: true,
                summary: format!(
                    "{} refreshed and queried Nginx {} through {}",
                    manager.name(),
                    selected.channel,
                    selected.uri
                ),
    ...
    fn restore(
        ...
        Ok(RestoreResult {
            ...
            summary: format!(
                "restored {} Nginx package repository files from {}",
    ...
    fn from_name(value: &str) -> Result<Self, AdapterError> {
        match value {
            ...
            _ => Err(AdapterError::InvalidConfiguration(
                "unknown Nginx package manager".into(),
            )),
        }
    }
}
...
fn require_linux(context: &SystemContext) -> Result<(), AdapterError> {
    if context.os != OperatingSystem::Linux {
        return Err(AdapterError::Unsupported(
            "Nginx package adapter supports Linux only".into(),
        ));
    }
    Ok(())
}
...
fn require_scope(scope: ConfigurationScope) -> Result<(), AdapterError> {
    if scope != ConfigurationScope::System {
        return Err(AdapterError::Unsupported(
            "Nginx package repositories require system scope".into(),
        ));
    }
    Ok(())
}
...
fn require_current(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    if current.tool_id != "nginx" || current.scope != ConfigurationScope::System {
        return Err(AdapterError::InvalidConfiguration(
            "Nginx package operation received another tool or scope".into(),
        ));
    }
    Ok(())
}
...
fn manager(runtime: &dyn Runtime) -> Result<Manager, AdapterError> {
    if runtime.command_exists("apt-get") && runtime.command_exists("apt-cache") {
        ...
        Err(AdapterError::Unsupported(
            "Nginx packages require apt-get/apt-cache or dnf".into(),
        ))
    }
}
...
fn read_documents(
    ...
    let (main, directory, target, extensions) = match manager {
        Manager::Apt => (
            ...
            PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-nginx.sources"),
            &["list", "sources"][..],
        ),
        Manager::Rpm => (
            None,
            PathBuf::from("/etc/yum.repos.d"),
            PathBuf::from("/etc/yum.repos.d/mirrorswitch-nginx.repo"),
            &["repo"][..],
        ),
    };
    ...
    for stanza in text.split("\n\n") {
        ...
        for line in stanza.lines() {
            ...
            if let Some(value) = active.strip_prefix("URIs:") {
                for uri in value.split_whitespace().filter(|uri| is_nginx_apt_uri(uri)) {
                    if deb822_uri
                        .replace(uri.trim_end_matches('/').into())
                        .is_some()
                    {
                        return Err(AdapterError::InvalidConfiguration(format!(
                            "multiple Nginx APT URIs exist in {}",
                            path.display()
                        )));
                    }
                }
            ...
            for (index, value) in fields.iter().enumerate() {
                if !is_nginx_apt_uri(value) {
                    continue;
                }
                let suite = fields.get(index + 1).ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "Nginx APT source in {} has no suite",
                        path.display()
                    ))
                })?;
                matches.push((
        ...
        if let Some(uri) = deb822_uri {
            matches.push((
                ...
                deb822_suite.ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "Nginx deb822 source in {} has no suite",
                        path.display()
                    ))
                })?,
                deb822_components,
...
fn apt_identity(
    ...
    if components != ["nginx"] {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Nginx APT source in {} must use the nginx component",
            path.display()
        )));
    }
    ...
    let channel = if uri.contains("/mainline/") {
        "mainline"
    } else {
        "stable"
    };
    if family.is_empty() {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Nginx APT source in {} has an invalid version or family",
            path.display()
        )));
    }
...
fn parse_rpm_repository(text: &str, path: &Path) -> Result<Option<RepositorySource>, AdapterError> {
    ...
    let matches = values
        ...
        .filter_map(|(repo_id, values)| {
            ...
            (values.get("enabled").is_none_or(|value| value != "0") && is_nginx_rpm_uri(uri))
                .then(|| (repo_id.clone(), uri.trim_end_matches('/').to_owned()))
        })
        .collect::<Vec<_>>();
    if matches.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "multiple Nginx Community server RPM repositories exist in {}",
            path.display()
        )));
    }
    matches
        ...
        .map(|(repo_id, uri)| {
            ...
            let (channel, release) = rpm_identity(uri).ok_or_else(|| {
                AdapterError::InvalidConfiguration(format!(
                    "Nginx RPM URI in {} has an unknown channel or release",
                    path.display()
                ))
            })?;
            Ok(RepositorySource {
...
fn is_nginx_apt_uri(value: &str) -> bool {
    let value = value.trim_end_matches('/');
    (value.starts_with("https://nginx.org/packages/")
        || value.starts_with("http://nginx.org/packages/")
...
fn is_nginx_rpm_uri(value: &str) -> bool {
    let value = value.trim_end_matches('/');
    (value.starts_with("http://nginx.org/packages/")
        || value.starts_with("https://nginx.org/packages/")
        || ACTIONABLE_ENDPOINTS
...
fn rpm_identity(value: &str) -> Option<(String, String)> {
    let channel = if value.contains("/mainline/") {
        "mainline"
    } else {
        "stable"
    };
...
fn unique_configured_channel(
    ...
    if values.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(
            "configured Nginx repositories cross product channels".into(),
        ));
    }
...
fn choose_target(manager: Manager, documents: &[ObservedDocument]) -> PathBuf {
    documents
        ...
        .unwrap_or_else(|| match manager {
            Manager::Apt => PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-nginx.sources"),
            Manager::Rpm => PathBuf::from("/etc/yum.repos.d/mirrorswitch-nginx.repo"),
        })
}

fn is_managed_path(path: &Path) -> bool {
    path.file_name().is_some_and(|name| {
        name == "mirrorswitch-nginx.sources" || name == "mirrorswitch-nginx.repo"
    })
}

fn installed_version(runtime: &dyn Runtime) -> Result<Option<String>, AdapterError> {
    if !runtime.command_exists("nginx") {
        return Ok(None);
    }
    let output = runtime.run("nginx", &["-v".into()])?;
    if !output.status.success() {
        return Ok(None);
    }
    let output = [output.stdout, output.stderr].concat();
    let output = String::from_utf8(output)
        .map_err(|_| AdapterError::Runtime("nginx -v is not UTF-8".into()))?;
    Ok(output
        .split_whitespace()
        .map(|token| token.trim_matches([',', ';']))
        .find(|token| version_pair(token.trim_start_matches("nginx/")).is_some())
        .map(|token| token.trim_start_matches("nginx/").to_owned()))
}
...
fn channel_from_version(value: &str) -> Result<&'static str, AdapterError> {
    match version_pair(value) {
        Some((1, 30)) => Ok("stable"),
        Some((1, 31)) => Ok("mainline"),
        Some((major, minor)) => Err(AdapterError::Unsupported(format!(
            "installed Nginx {major}.{minor} is outside reviewed 1.30/1.31 coverage"
        ))),
        None => Err(AdapterError::Unsupported(format!(
            "installed Nginx version {value} is invalid"
        ))),
    }
}
...
fn platform(context: &SystemContext, manager: Manager) -> Result<Platform, AdapterError> {
    let distribution = context.distribution.as_ref().ok_or_else(|| {
        AdapterError::Unsupported("Nginx packages require a detected Linux distribution".into())
    })?;
    ...
    match manager {
        Manager::Apt => {
            let suite = distribution.version_codename.as_deref().ok_or_else(|| {
                AdapterError::Unsupported("Nginx APT requires a distribution codename".into())
            })?;
            ...
            if !supported {
                return Err(AdapterError::Unsupported(format!(
                    "Nginx stable APT coverage excludes {} {}",
        ...
        Manager::Rpm => {
            ...
            {
                return Err(AdapterError::Unsupported(format!(
                    "Nginx stable RPM coverage is EL9 only, not {} {}",
...
fn mapped_uri(
    ...
    let prefix = if channel == "mainline" {
        "mainline/"
    } else {
        ""
    };
    match (manager, provider) {
        (Manager::Apt, "nju") => Ok(format!("{endpoint}/{prefix}{}", platform.family)),
        (Manager::Apt, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed Nginx APT repository".into(),
        )),
        (Manager::Rpm, "nju") => Ok(format!(
            "{endpoint}/{prefix}rhel/9/{}",
            platform.architecture
        )),
        (Manager::Rpm, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed Nginx RPM repository".into(),
        )),
    }
}
...
fn platform_tag(platform: &Platform) -> Result<&'static str, AdapterError> {
    match (platform.family.as_str(), platform.release.as_str()) {
        ...
        _ => Err(AdapterError::Unsupported(
            "Nginx package tag is unavailable for this target".into(),
        )),
    }
}

fn render_new(manager: Manager, mapped_uri: &str, platform: &Platform, channel: &str) -> String {
    match manager {
        Manager::Apt => format!(
...
fn rewrite_existing(
    ...
    let source = parse_repository(manager, text, path)?.ok_or_else(|| {
        AdapterError::InvalidConfiguration("selected Nginx repository disappeared".into())
    })?;
    if source.channel != channel {
        return Err(AdapterError::Unsupported(
            "Nginx repository channel changed during planning".into(),
        ));
    }
...
fn replace_active_once(
    ...
    if ranges.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Nginx repository URI in {} is missing or duplicated",
            path.display()
        )));
    }
...
fn selected_endpoint(selections: &[MirrorSelection]) -> Result<(String, String), AdapterError> {
    if selections.len() != 1
        || selections[0].tool_id != "nginx"
        || selections[0].upstream_id != UPSTREAM
    {
        return Err(AdapterError::InvalidConfiguration(
            "Nginx packages require exactly one repository selection".into(),
        ));
    }
    ...
    let role_url = |role| -> Result<String, AdapterError> {
        ...
        if endpoints.len() != 1 {
            return Err(AdapterError::InvalidConfiguration(format!(
                "Nginx package selection needs one HTTPS {role:?} endpoint"
            )));
        }
        normalized_endpoint(&endpoints[0].url).ok_or_else(|| {
            AdapterError::InvalidConfiguration("Nginx package endpoint is unsafe".into())
        })
    };
    let index = role_url(EndpointRole::Index)?;
    if role_url(EndpointRole::Metadata)? != index || role_url(EndpointRole::Packages)? != index {
        return Err(AdapterError::InvalidConfiguration(
            "Nginx package index, metadata, and package endpoints must match".into(),
        ));
    }
    ...
    {
        return Err(AdapterError::InvalidConfiguration(
            "Nginx package provider and endpoint are not reviewed".into(),
        ));
    }
...
fn validate_policy(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    for source in &current.sources {
        match metadata(source, "kind")? {
            "multiple-modern-repositories" => {
                return Err(AdapterError::InvalidConfiguration(
                    "multiple modern Nginx package repositories are active".into(),
                ));
            }
            "coverage-mismatch" => {
                return Err(AdapterError::Unsupported(
                    "Nginx repository distribution, release, or channel does not match the host"
                        .into(),
                ));
            }
            "keyring-missing" => {
                return Err(AdapterError::Unsupported(
                    "Nginx APT keyring is missing; repository security cannot be preserved".into(),
                ));
            }
            "gpgcheck-disabled" => {
                return Err(AdapterError::Unsupported(
                    "Nginx RPM repository has gpgcheck disabled".into(),
                ));
            }
            "gpgkey-missing" => {
                return Err(AdapterError::Unsupported(
                    "Nginx RPM repository has no GPG key configured".into(),
                ));
            }
            "managed-target-conflict" => {
                return Err(AdapterError::Unsupported(
                    "Nginx package target belongs to another MirrorSwitch format".into(),
                ));
            }
            _ => {}
        }
...
fn verify_apt(runtime: &dyn Runtime, target: &Path) -> Result<String, AdapterError> {
    ...
    command_output(runtime.run("apt-get", &update)?, "Nginx APT refresh")?;
    let mut query = Vec::new();
    for option in &options {
        query.extend(["-o".into(), option.clone()]);
    }
    query.extend(["policy".into(), "nginx".into()]);
    command_output(runtime.run("apt-cache", &query)?, "Nginx APT query")
}
...
fn verify_rpm(
    ...
    let repo_id = repo_id.unwrap_or("mirrorswitch-nginx");
    ...
    command_output(runtime.run("dnf", &refresh)?, "Nginx RPM refresh")?;
    ...
    command_output(runtime.run("dnf", &query)?, "Nginx RPM query")
}
...
fn find_document<'a>(
    ...
    if matches.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Nginx package state must contain one {format} document"
        )));
    }
    Ok(matches[0])
}
...
fn current_value(current: &CurrentConfiguration, kind: &str) -> Result<String, AdapterError> {
    ...
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Nginx package state has ambiguous {kind}"
        )));
    }
    Ok(values[0].clone())
}
...
fn snapshot_source(kind: &str, value: &str) -> ConfiguredSource {
    ConfiguredSource {
        upstream_id: None,
        url: format!("nginx-package-snapshot:{value}"),
        enabled: true,
        metadata: BTreeMap::from([
...
fn metadata<'a>(source: &'a ConfiguredSource, key: &str) -> Result<&'a str, AdapterError> {
    let values = source.metadata.get(key).ok_or_else(|| {
        AdapterError::InvalidConfiguration(format!(
            "Nginx package source is missing {key} metadata"
        ))
    })?;
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Nginx package source has ambiguous {key} metadata"
        )));
    }
    Ok(&values[0])
}
...
fn validate_path(path: &Path, kind: &str) -> Result<(), AdapterError> {
    ...
    {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Nginx package {kind} path {} is unsafe",
            path.display()
        )));
    }
    Ok(())
...
fn utf8<'a>(path: &Path, contents: &'a [u8]) -> Result<&'a str, AdapterError> {
    std::str::from_utf8(contents).map_err(|_| {
        AdapterError::InvalidConfiguration(format!(
            "Nginx package configuration {} is not UTF-8",
            path.display()
        ))
    })
}
