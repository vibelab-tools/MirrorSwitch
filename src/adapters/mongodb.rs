...
const MANAGED_MARKER: &str = "# Managed by MirrorSwitch: MongoDB package repository v1";
const DEFAULT_KEYRING: &str = "/usr/share/keyrings/mongodb-server-8.0.gpg";
const CHANNEL: &str = "8.0";
...
impl Adapter for MongoDbAdapter {
    ...
    fn detect(
            ...
            Some(version) => {
                    ...
                    .is_some_and(|configured| configured != channel)
                {
                    return Err(AdapterError::Unsupported(format!(
                        "installed MongoDB {version} does not match configured channel {}",
        ...
        if channel != CHANNEL {
            return Err(AdapterError::Unsupported(format!(
                "MongoDB channel {channel} is outside reviewed Community 8.0 coverage"
            )));
        }
        ...
        if configured > 1 {
            return Err(AdapterError::InvalidConfiguration(
                "multiple MongoDB Community server repositories are active".into(),
            ));
        }
        ...
        Ok(Some(DetectedTool {
            ...
            evidence: vec![
                ...
                format!("target MongoDB channel is {channel}"),
                format!(
                    "installed MongoDB version is {}",
                    installed.as_deref().unwrap_or("not detected")
                ),
                format!("configured MongoDB Community server repositories: {configured}"),
                ...
                "MongoDB editions, majors, private repositories, and service data are separate surfaces".into(),
            ],
        }))
    }
    ...
    fn read_current(
        ...
        require_linux(context)?;
        require_scope(scope)?;
        if detected.tool_id != "mongodb" {
            return Err(AdapterError::InvalidConfiguration(
                "MongoDB package read received another tool's detection result".into(),
            ));
        }
        let manager = manager(runtime)?;
        ...
        let channel = match installed.as_deref() {
            ...
            None => configured_channel.clone().ok_or_else(|| {
                AdapterError::Unsupported(
                    "MongoDB channel cannot be inferred from the binary or repository".into(),
                )
            })?,
        };
        if channel != CHANNEL {
            ...
            .is_some_and(|configured| configured != channel)
        {
            return Err(AdapterError::Unsupported(
                "installed MongoDB version and configured server channel differ".into(),
            ));
        }
        let platform = platform(context, manager)?;
        ...
        if detected.version.as_deref() != Some(effective_version) {
            return Err(AdapterError::Conflict(
                "MongoDB package target version changed after detection".into(),
            ));
        }
        ...
        for document in observed {
                ...
                match manager {
                    Manager::Apt => {
                        let keyring = source.signed_by.as_deref().ok_or_else(|| {
                            AdapterError::Unsupported(
                                "modern MongoDB APT source has no explicit Signed-By keyring"
                                    .into(),
                            )
                        })?;
            ...
            documents.push(ConfigurationDocument {
                ...
                format: if document.path == target {
                    "mongodb-package-target".into()
                } else {
                    "mongodb-package-read-only".into()
                },
    ...
    fn plan(
        ...
        let target = find_document(current, "mongodb-package-target")?;
        ...
        let changes = if target.contents == rendered {
            ...
            vec![PlannedFileChange {
                ...
                new_mode: None,
                summary: format!(
                    "map only the MongoDB {channel} {} package channel while preserving GPG, pinning, credentials, and unrelated repositories",
                    manager.name()
                ),
            }]
        };
        ...
        let result = (|| {
            ...
            if !receipt.changed_targets.contains(&physical) {
                return Err(AdapterError::Verification(
                    "MongoDB package transaction contains no known target".into(),
                ));
            }
            let selected = documents
                ...
                .ok_or_else(|| {
                    AdapterError::Verification(
                        "MongoDB package repository is not active after apply".into(),
                    )
                })?;
            ...
            if !output.contains("mongodb-org-server") || !output.contains("8.0") {
                return Err(AdapterError::Verification(
                    "system package manager did not expose mongodb-org-server 8.0".into(),
                ));
            }
            Ok(VerificationResult {
                valid: true,
                summary: format!(
                    "{} refreshed and queried mongodb-org-server from MongoDB {} through {}",
                    manager.name(),
                    selected.channel,
                    selected.uri
                ),
    ...
    fn restore(
        ...
        Ok(RestoreResult {
            ...
            summary: format!(
                "restored {} MongoDB package repository files from {}",
    ...
    fn from_name(value: &str) -> Result<Self, AdapterError> {
        match value {
            ...
            _ => Err(AdapterError::InvalidConfiguration(
                "unknown MongoDB package manager".into(),
            )),
        }
    }
}
...
fn require_linux(context: &SystemContext) -> Result<(), AdapterError> {
    if context.os != OperatingSystem::Linux {
        return Err(AdapterError::Unsupported(
            "MongoDB package adapter supports Linux only".into(),
        ));
    }
    Ok(())
}
...
fn require_scope(scope: ConfigurationScope) -> Result<(), AdapterError> {
    if scope != ConfigurationScope::System {
        return Err(AdapterError::Unsupported(
            "MongoDB package repositories require system scope".into(),
        ));
    }
    Ok(())
}
...
fn require_current(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    if current.tool_id != "mongodb" || current.scope != ConfigurationScope::System {
        return Err(AdapterError::InvalidConfiguration(
            "MongoDB package operation received another tool or scope".into(),
        ));
    }
    Ok(())
}
...
fn manager(runtime: &dyn Runtime) -> Result<Manager, AdapterError> {
    if runtime.command_exists("apt-get") && runtime.command_exists("apt-cache") {
        ...
        Err(AdapterError::Unsupported(
            "MongoDB packages require apt-get/apt-cache or dnf".into(),
        ))
    }
}
...
fn read_documents(
    ...
    let (main, directory, target, extensions) = match manager {
        Manager::Apt => (
            ...
            PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-mongodb.sources"),
            &["list", "sources"][..],
        ),
        Manager::Rpm => (
            None,
            PathBuf::from("/etc/yum.repos.d"),
            PathBuf::from("/etc/yum.repos.d/mirrorswitch-mongodb.repo"),
            &["repo"][..],
        ),
    };
...
fn parse_apt_repository(text: &str, path: &Path) -> Result<Option<RepositorySource>, AdapterError> {
    ...
    for stanza in text.split("\n\n") {
        ...
        for line in stanza.lines() {
            ...
            if let Some(value) = active.strip_prefix("URIs:") {
                for uri in value
                    .split_whitespace()
                    .filter(|uri| is_mongodb_apt_uri(uri))
                {
                    if deb822_uri
                        ...
                        .is_some()
                    {
                        return Err(AdapterError::InvalidConfiguration(format!(
                            "multiple MongoDB APT URIs exist in {}",
                            path.display()
                        )));
                    }
                }
            ...
            for (index, value) in fields.iter().enumerate() {
                if !is_mongodb_apt_uri(value) {
                    continue;
                }
                let suite = fields.get(index + 1).ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "MongoDB APT source in {} has no suite",
                        path.display()
                    ))
                })?;
                matches.push((
        ...
        if let Some(uri) = deb822_uri {
            matches.push((
                ...
                deb822_suite.ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "MongoDB deb822 source in {} has no suite",
                        path.display()
                    ))
                })?,
                deb822_components,
    ...
    matches
        ...
        .map(|(uri, suite, components, signed_by)| {
            let (family, release, channel) = apt_identity(uri, suite, components, path)?;
            Ok(RepositorySource {
                uri: uri.clone(),
                channel,
                family,
...
fn apt_identity(
    ...
    let family = if uri.ends_with("/ubuntu") {
        "ubuntu"
    } else if uri.ends_with("/debian") {
        "debian"
    } else {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MongoDB APT source in {} has an unknown distribution family",
            path.display()
        )));
    };
    let (release, channel) = suite.split_once("/mongodb-org/").ok_or_else(|| {
        AdapterError::InvalidConfiguration(format!(
            "MongoDB APT source in {} has no mongodb-org channel",
            path.display()
        ))
    })?;
    if version_pair(channel).is_none() {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MongoDB APT source in {} has an invalid channel",
            path.display()
        )));
    }
    let expected_component = if family == "debian" {
        "main"
    } else {
        "multiverse"
    };
    if components != [expected_component] {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MongoDB APT source in {} has unexpected components",
            path.display()
        )));
    }
    Ok((family.into(), release.into(), channel.into()))
}
...
fn parse_rpm_repository(text: &str, path: &Path) -> Result<Option<RepositorySource>, AdapterError> {
    ...
    let matches = values
        ...
        .filter_map(|(repo_id, values)| {
            ...
            (values.get("enabled").is_none_or(|value| value != "0") && is_mongodb_rpm_uri(uri))
                .then(|| (repo_id.clone(), uri.trim_end_matches('/').to_owned()))
        })
        .collect::<Vec<_>>();
    if matches.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "multiple MongoDB Community server RPM repositories exist in {}",
            path.display()
        )));
    }
    matches
        ...
        .map(|(repo_id, uri)| {
            ...
            let (channel, release) = rpm_identity(uri).ok_or_else(|| {
                AdapterError::InvalidConfiguration(format!(
                    "MongoDB RPM URI in {} has an unknown channel or release",
                    path.display()
                ))
            })?;
            Ok(RepositorySource {
...
fn is_mongodb_apt_uri(value: &str) -> bool {
    [
        "http://repo.mongodb.org/apt/debian",
        "https://repo.mongodb.org/apt/debian",
        "http://repo.mongodb.org/apt/ubuntu",
        "https://repo.mongodb.org/apt/ubuntu",
        "https://mirrors.aliyun.com/mongodb/apt/debian",
        "https://mirrors.aliyun.com/mongodb/apt/ubuntu",
        "https://repo.huaweicloud.com/mongodb/apt/debian",
        "https://repo.huaweicloud.com/mongodb/apt/ubuntu",
        "https://mirrors.nju.edu.cn/mongodb/apt/debian",
        "https://mirrors.nju.edu.cn/mongodb/apt/ubuntu",
        "https://mirrors.tuna.tsinghua.edu.cn/mongodb/apt/debian",
        "https://mirrors.tuna.tsinghua.edu.cn/mongodb/apt/ubuntu",
    ]
    .contains(&value.trim_end_matches('/'))
}

fn is_mongodb_rpm_uri(value: &str) -> bool {
    let value = value.trim_end_matches('/');
    (value.starts_with("http://repo.mongodb.org/yum/")
        || value.starts_with("https://repo.mongodb.org/yum/")
        || ACTIONABLE_ENDPOINTS
            .iter()
            .any(|(_, endpoint)| value.starts_with(&format!("{endpoint}/yum/"))))
        && (value.contains("/mongodb-org/8.0/") || value.contains("/el9-8.0"))
}

fn rpm_identity(value: &str) -> Option<(&'static str, &'static str)> {
    (value.contains("/redhat/9/mongodb-org/8.0/") || value.contains("/el9-8.0"))
        .then_some((CHANNEL, "9"))
}
...
fn unique_configured_channel(
    ...
    if values.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(
            "configured MongoDB repositories cross product channels".into(),
        ));
    }
...
fn choose_target(manager: Manager, documents: &[ObservedDocument]) -> PathBuf {
    documents
        ...
        .unwrap_or_else(|| match manager {
            Manager::Apt => PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-mongodb.sources"),
            Manager::Rpm => PathBuf::from("/etc/yum.repos.d/mirrorswitch-mongodb.repo"),
        })
}

fn is_managed_path(path: &Path) -> bool {
    path.file_name().is_some_and(|name| {
        name == "mirrorswitch-mongodb.sources" || name == "mirrorswitch-mongodb.repo"
    })
}

fn installed_version(runtime: &dyn Runtime) -> Result<Option<String>, AdapterError> {
    if !runtime.command_exists("mongod") {
        return Ok(None);
    }
    let output = runtime.run("mongod", &["--version".into()])?;
    if !output.status.success() {
        return Ok(None);
    }
    let output = String::from_utf8(output.stdout)
        .map_err(|_| AdapterError::Runtime("mongod --version is not UTF-8".into()))?;
    Ok(output
        .split_whitespace()
...
fn channel_from_version(value: &str) -> Result<&'static str, AdapterError> {
    match version_pair(value) {
        ...
        Some((major, minor)) => Err(AdapterError::Unsupported(format!(
            "installed MongoDB {major}.{minor} is outside reviewed Community 8.0 coverage"
        ))),
        None => Err(AdapterError::Unsupported(format!(
            "installed MongoDB version {value} is invalid"
        ))),
    }
}
...
fn platform(context: &SystemContext, manager: Manager) -> Result<Platform, AdapterError> {
    let distribution = context.distribution.as_ref().ok_or_else(|| {
        AdapterError::Unsupported("MongoDB packages require a detected Linux distribution".into())
    })?;
    ...
    match manager {
        Manager::Apt => {
            let suite = distribution.version_codename.as_deref().ok_or_else(|| {
                AdapterError::Unsupported("MongoDB APT requires a distribution codename".into())
            })?;
            ...
            if !supported {
                return Err(AdapterError::Unsupported(format!(
                    "MongoDB 8.0 APT coverage excludes {} {}",
        ...
        Manager::Rpm => {
            ...
            {
                return Err(AdapterError::Unsupported(format!(
                    "MongoDB 8.0 RPM coverage is EL9 only, not {} {}",
...
fn mapped_uri(
    ...
    match (manager, provider) {
        (Manager::Apt, "huaweicloud" | "nju" | "tuna")
            if platform.family != "debian" || platform.architecture != "arm64" =>
        {
            Ok(format!("{endpoint}/apt/{}", platform.family))
        }
        (Manager::Apt, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider lacks the requested MongoDB APT package set".into(),
        )),
        (Manager::Rpm, "huaweicloud" | "nju" | "tuna") if platform.architecture == "x86_64" => {
            Ok(format!("{endpoint}/yum/el{}-8.0", platform.release))
        }
        (Manager::Rpm, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed MongoDB RPM repository".into(),
        )),
    }
}
...
fn representative_package_name(
    ...
    match (
        ...
        (Manager::Apt, _, _, "amd64") => Ok("mongodb-org-server_8.0.28_amd64.deb"),
        (Manager::Apt, _, _, "arm64") => Ok("mongodb-org-server_8.0.28_arm64.deb"),
        (Manager::Rpm, "el", "9", "x86_64") => Ok("mongodb-org-server-8.0.28-1.el9.x86_64.rpm"),
        (Manager::Rpm, "el", "9", "aarch64") => Ok("mongodb-org-server-8.0.28-1.el9.aarch64.rpm"),
        _ => Err(AdapterError::Unsupported(
            "MongoDB representative package is unavailable for this target".into(),
        )),
    }
}
...
fn rewrite_existing(
    ...
    let source = parse_repository(manager, text, path)?.ok_or_else(|| {
        AdapterError::InvalidConfiguration("selected MongoDB repository disappeared".into())
    })?;
    if source.channel != channel {
        return Err(AdapterError::Unsupported(
            "MongoDB repository channel changed during planning".into(),
        ));
    }
...
fn replace_active_once(
    ...
    if ranges.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MongoDB repository URI in {} is missing or duplicated",
            path.display()
        )));
    }
...
fn selected_endpoint(selections: &[MirrorSelection]) -> Result<(String, String), AdapterError> {
    if selections.len() != 1
        || selections[0].tool_id != "mongodb"
        || selections[0].upstream_id != UPSTREAM
    {
        return Err(AdapterError::InvalidConfiguration(
            "MongoDB packages require exactly one repository selection".into(),
        ));
    }
    ...
    let role_url = |role| -> Result<String, AdapterError> {
        ...
        if endpoints.len() != 1 {
            return Err(AdapterError::InvalidConfiguration(format!(
                "MongoDB package selection needs one HTTPS {role:?} endpoint"
            )));
        }
        normalized_endpoint(&endpoints[0].url).ok_or_else(|| {
            AdapterError::InvalidConfiguration("MongoDB package endpoint is unsafe".into())
        })
    };
    let index = role_url(EndpointRole::Index)?;
    if role_url(EndpointRole::Metadata)? != index || role_url(EndpointRole::Packages)? != index {
        return Err(AdapterError::InvalidConfiguration(
            "MongoDB package index, metadata, and package endpoints must match".into(),
        ));
    }
    ...
    {
        return Err(AdapterError::InvalidConfiguration(
            "MongoDB package provider and endpoint are not reviewed".into(),
        ));
    }
...
fn validate_policy(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    for source in &current.sources {
        match metadata(source, "kind")? {
            "multiple-modern-repositories" => {
                return Err(AdapterError::InvalidConfiguration(
                    "multiple modern MongoDB package repositories are active".into(),
                ));
            }
            "coverage-mismatch" => {
                return Err(AdapterError::Unsupported(
                    "MongoDB repository distribution, release, or channel does not match the host"
                        .into(),
                ));
            }
            "keyring-missing" => {
                return Err(AdapterError::Unsupported(
                    "MongoDB APT keyring is missing; repository security cannot be preserved"
                        .into(),
                ));
            }
            "gpgcheck-disabled" => {
                return Err(AdapterError::Unsupported(
                    "MongoDB RPM repository has gpgcheck disabled".into(),
                ));
            }
            "gpgkey-missing" => {
                return Err(AdapterError::Unsupported(
                    "MongoDB RPM repository has no GPG key configured".into(),
                ));
            }
            "managed-target-conflict" => {
                return Err(AdapterError::Unsupported(
                    "MongoDB package target belongs to another MirrorSwitch format".into(),
                ));
            }
            _ => {}
        }
...
fn verify_apt(runtime: &dyn Runtime, target: &Path) -> Result<String, AdapterError> {
    ...
    command_output(runtime.run("apt-get", &update)?, "MongoDB APT refresh")?;
    let mut query = Vec::new();
    for option in &options {
        query.extend(["-o".into(), option.clone()]);
    }
    query.extend(["policy".into(), "mongodb-org-server".into()]);
    command_output(runtime.run("apt-cache", &query)?, "MongoDB APT query")
}
...
fn verify_rpm(
    ...
    let repo_id = repo_id.unwrap_or("mirrorswitch-mongodb");
    ...
    command_output(runtime.run("dnf", &refresh)?, "MongoDB RPM refresh")?;
    let mut query = common.to_vec();
    query.extend([
        "-q".into(),
        "list".into(),
        "--showduplicates".into(),
        "mongodb-org-server".into(),
    ]);
    command_output(runtime.run("dnf", &query)?, "MongoDB RPM query")
}
...
fn find_document<'a>(
    ...
    if matches.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MongoDB package state must contain one {format} document"
        )));
    }
    Ok(matches[0])
}
...
fn current_value(current: &CurrentConfiguration, kind: &str) -> Result<String, AdapterError> {
    ...
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MongoDB package state has ambiguous {kind}"
        )));
    }
    Ok(values[0].clone())
}
...
fn snapshot_source(kind: &str, value: &str) -> ConfiguredSource {
    ConfiguredSource {
        upstream_id: None,
        url: format!("mongodb-package-snapshot:{value}"),
        enabled: true,
        metadata: BTreeMap::from([
...
fn metadata<'a>(source: &'a ConfiguredSource, key: &str) -> Result<&'a str, AdapterError> {
    let values = source.metadata.get(key).ok_or_else(|| {
        AdapterError::InvalidConfiguration(format!(
            "MongoDB package source is missing {key} metadata"
        ))
    })?;
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MongoDB package source has ambiguous {key} metadata"
        )));
    }
    Ok(&values[0])
}
...
fn validate_path(path: &Path, kind: &str) -> Result<(), AdapterError> {
    ...
    {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MongoDB package {kind} path {} is unsafe",
            path.display()
        )));
    }
    Ok(())
...
fn utf8<'a>(path: &Path, contents: &'a [u8]) -> Result<&'a str, AdapterError> {
    std::str::from_utf8(contents).map_err(|_| {
        AdapterError::InvalidConfiguration(format!(
            "MongoDB package configuration {} is not UTF-8",
            path.display()
        ))
    })
}
