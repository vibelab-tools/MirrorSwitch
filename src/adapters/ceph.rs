...
const MANAGED_MARKER: &str = "# Managed by MirrorSwitch: Ceph package repository v1";
const DEFAULT_KEYRING: &str = "/usr/share/keyrings/ceph-release.gpg";
const CHANNEL: &str = "squid";
...
impl Adapter for CephAdapter {
    ...
    fn detect(
            ...
            Some(version) => {
                    ...
                    .is_some_and(|configured| configured != channel)
                {
                    return Err(AdapterError::Unsupported(format!(
                        "installed Ceph {version} does not match configured channel {}",
        ...
        if channel != CHANNEL {
            return Err(AdapterError::Unsupported(format!(
                "Ceph channel {channel} is outside reviewed squid LTS coverage"
            )));
        }
        ...
        if configured > 1 {
            return Err(AdapterError::InvalidConfiguration(
                "multiple Ceph Community server repositories are active".into(),
            ));
        }
        ...
        Ok(Some(DetectedTool {
            ...
            evidence: vec![
                ...
                format!("target Ceph channel is {channel}"),
                format!(
                    "installed Ceph version is {}",
                    installed.as_deref().unwrap_or("not detected")
                ),
                format!("configured Ceph Community server repositories: {configured}"),
                ...
                "Ceph majors, MySQL repositories, private repositories, and service data are separate surfaces".into(),
            ],
        }))
    }
    ...
    fn read_current(
        ...
        require_linux(context)?;
        require_scope(scope)?;
        if detected.tool_id != "ceph" {
            return Err(AdapterError::InvalidConfiguration(
                "Ceph package read received another tool's detection result".into(),
            ));
        }
        let manager = manager(runtime)?;
        ...
        let channel = match installed.as_deref() {
            ...
            None => configured_channel.clone().ok_or_else(|| {
                AdapterError::Unsupported(
                    "Ceph channel cannot be inferred from the binary or repository".into(),
                )
            })?,
        };
        if channel != CHANNEL {
            ...
            .is_some_and(|configured| configured != channel)
        {
            return Err(AdapterError::Unsupported(
                "installed Ceph version and configured server channel differ".into(),
            ));
        }
        let platform = platform(context, manager)?;
        ...
        if detected.version.as_deref() != Some(effective_version) {
            return Err(AdapterError::Conflict(
                "Ceph package target version changed after detection".into(),
            ));
        }
        ...
        for document in observed {
                ...
                match manager {
                    Manager::Apt => {
                        let keyring = source.signed_by.as_deref().ok_or_else(|| {
                            AdapterError::Unsupported(
                                "modern Ceph APT source has no explicit Signed-By keyring".into(),
                            )
                        })?;
            ...
            documents.push(ConfigurationDocument {
                ...
                format: if document.path == target {
                    "ceph-package-target".into()
                } else {
                    "ceph-package-read-only".into()
                },
    ...
    fn plan(
        ...
        let target = find_document(current, "ceph-package-target")?;
        ...
        let changes = if target.contents == rendered {
            ...
            vec![PlannedFileChange {
                ...
                new_mode: None,
                summary: format!(
                    "map only the Ceph {channel} {} package channel while preserving GPG, pinning, credentials, and unrelated repositories",
                    manager.name()
                ),
            }]
        };
        ...
        let result = (|| {
            ...
            if !receipt.changed_targets.contains(&physical) {
                return Err(AdapterError::Verification(
                    "Ceph package transaction contains no known target".into(),
                ));
            }
            let selected = documents
                ...
                .ok_or_else(|| {
                    AdapterError::Verification(
                        "Ceph package repository is not active after apply".into(),
                    )
                })?;
            ...
            if !normalized.contains("ceph") || !normalized.contains("19.") {
                return Err(AdapterError::Verification(
                    "system package manager did not expose Ceph Squid 19.x".into(),
                ));
            }
            Ok(VerificationResult {
                valid: true,
                summary: format!(
                    "{} refreshed and queried Ceph {} through {}",
                    manager.name(),
                    selected.channel,
                    selected.uri
                ),
    ...
    fn restore(
        ...
        Ok(RestoreResult {
            ...
            summary: format!(
                "restored {} Ceph package repository files from {}",
    ...
    fn from_name(value: &str) -> Result<Self, AdapterError> {
        match value {
            ...
            _ => Err(AdapterError::InvalidConfiguration(
                "unknown Ceph package manager".into(),
            )),
        }
    }
}
...
fn require_linux(context: &SystemContext) -> Result<(), AdapterError> {
    if context.os != OperatingSystem::Linux {
        return Err(AdapterError::Unsupported(
            "Ceph package adapter supports Linux only".into(),
        ));
    }
    Ok(())
}
...
fn require_scope(scope: ConfigurationScope) -> Result<(), AdapterError> {
    if scope != ConfigurationScope::System {
        return Err(AdapterError::Unsupported(
            "Ceph package repositories require system scope".into(),
        ));
    }
    Ok(())
}
...
fn require_current(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    if current.tool_id != "ceph" || current.scope != ConfigurationScope::System {
        return Err(AdapterError::InvalidConfiguration(
            "Ceph package operation received another tool or scope".into(),
        ));
    }
    Ok(())
}
...
fn manager(runtime: &dyn Runtime) -> Result<Manager, AdapterError> {
    if runtime.command_exists("apt-get") && runtime.command_exists("apt-cache") {
        ...
        Err(AdapterError::Unsupported(
            "Ceph packages require apt-get/apt-cache or dnf".into(),
        ))
    }
}
...
fn read_documents(
    ...
    let (main, directory, target, extensions) = match manager {
        Manager::Apt => (
            ...
            PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-ceph.sources"),
            &["list", "sources"][..],
        ),
        Manager::Rpm => (
            None,
            PathBuf::from("/etc/yum.repos.d"),
            PathBuf::from("/etc/yum.repos.d/mirrorswitch-ceph.repo"),
            &["repo"][..],
        ),
    };
...
fn parse_apt_repository(text: &str, path: &Path) -> Result<Option<RepositorySource>, AdapterError> {
    ...
    for stanza in text.split("\n\n") {
        ...
        for line in stanza.lines() {
            ...
            if let Some(value) = active.strip_prefix("URIs:") {
                for uri in value.split_whitespace().filter(|uri| is_ceph_apt_uri(uri)) {
                    if deb822_uri
                        .replace(uri.trim_end_matches('/').into())
                        .is_some()
                    {
                        return Err(AdapterError::InvalidConfiguration(format!(
                            "multiple Ceph APT URIs exist in {}",
                            path.display()
                        )));
                    }
                }
            ...
            for (index, value) in fields.iter().enumerate() {
                if !is_ceph_apt_uri(value) {
                    continue;
                }
                let suite = fields.get(index + 1).ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "Ceph APT source in {} has no suite",
                        path.display()
                    ))
                })?;
                matches.push((
        ...
        if let Some(uri) = deb822_uri {
            matches.push((
                ...
                deb822_suite.ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "Ceph deb822 source in {} has no suite",
                        path.display()
                    ))
                })?,
                deb822_components,
    ...
    matches
        ...
        .map(|(uri, suite, components, signed_by)| {
            let (family, channel) = apt_identity(uri, suite, components, path)?;
            Ok(RepositorySource {
                uri: uri.clone(),
                channel,
                family,
...
fn apt_identity(
    ...
    if components != ["main"] {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Ceph APT source in {} must use the main component",
            path.display()
        )));
    }
    let channel = uri
        ...
        .unwrap_or_default();
    let family = if suite == "bookworm" {
        "debian"
    } else {
        "ubuntu"
    };
    if channel.is_empty() || !matches!(suite, "bookworm" | "jammy") {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Ceph APT source in {} has an invalid version or family",
            path.display()
        )));
    }
...
fn parse_rpm_repository(text: &str, path: &Path) -> Result<Option<RepositorySource>, AdapterError> {
    ...
    let matches = values
        ...
        .filter_map(|(repo_id, values)| {
            ...
            (values.get("enabled").is_none_or(|value| value != "0") && is_ceph_rpm_uri(uri))
                .then(|| (repo_id.clone(), uri.trim_end_matches('/').to_owned()))
        })
        .collect::<Vec<_>>();
    if matches.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "multiple Ceph Community server RPM repositories exist in {}",
            path.display()
        )));
    }
    matches
        ...
        .map(|(repo_id, uri)| {
            ...
            let (channel, release) = rpm_identity(uri).ok_or_else(|| {
                AdapterError::InvalidConfiguration(format!(
                    "Ceph RPM URI in {} has an unknown channel or release",
                    path.display()
                ))
            })?;
            Ok(RepositorySource {
...
fn is_ceph_apt_uri(value: &str) -> bool {
    let value = value.trim_end_matches('/');
    value.starts_with("https://download.ceph.com/debian-")
        || value.starts_with("http://download.ceph.com/debian-")
        || ACTIONABLE_ENDPOINTS
            .iter()
            .any(|(_, endpoint)| value.starts_with(&format!("{endpoint}/debian-")))
}

fn is_ceph_rpm_uri(value: &str) -> bool {
    let value = value.trim_end_matches('/');
    (value.starts_with("http://download.ceph.com/rpm-")
        || value.starts_with("https://download.ceph.com/rpm-")
        || ACTIONABLE_ENDPOINTS
            .iter()
            .any(|(_, endpoint)| value.starts_with(&format!("{endpoint}/rpm-"))))
        && value.contains("/el9/")
}

fn rpm_identity(value: &str) -> Option<(String, String)> {
    let channel = value.rsplit_once("/rpm-")?.1.split('/').next()?;
    (!channel.is_empty() && value.contains("/el9/")).then(|| (channel.into(), "9".into()))
}
...
fn unique_configured_channel(
    ...
    if values.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(
            "configured Ceph repositories cross product channels".into(),
        ));
    }
...
fn choose_target(manager: Manager, documents: &[ObservedDocument]) -> PathBuf {
    documents
        ...
        .unwrap_or_else(|| match manager {
            Manager::Apt => PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-ceph.sources"),
            Manager::Rpm => PathBuf::from("/etc/yum.repos.d/mirrorswitch-ceph.repo"),
        })
}

fn is_managed_path(path: &Path) -> bool {
    path.file_name()
        .is_some_and(|name| name == "mirrorswitch-ceph.sources" || name == "mirrorswitch-ceph.repo")
}

fn installed_version(runtime: &dyn Runtime) -> Result<Option<String>, AdapterError> {
    if !runtime.command_exists("ceph") {
        return Ok(None);
    }
    let output = runtime.run("ceph", &["--version".into()])?;
    if !output.status.success() {
        return Ok(None);
    }
    let output = String::from_utf8(output.stdout)
        .map_err(|_| AdapterError::Runtime("ceph --version is not UTF-8".into()))?;
    Ok(output
        .split_whitespace()
...
fn channel_from_version(value: &str) -> Result<&'static str, AdapterError> {
    match version_pair(value) {
        ...
        Some((major, minor)) => Err(AdapterError::Unsupported(format!(
            "installed Ceph {major}.{minor} is outside reviewed Squid 19.x coverage"
        ))),
        None => Err(AdapterError::Unsupported(format!(
            "installed Ceph version {value} is invalid"
        ))),
    }
}
...
fn platform(context: &SystemContext, manager: Manager) -> Result<Platform, AdapterError> {
    let distribution = context.distribution.as_ref().ok_or_else(|| {
        AdapterError::Unsupported("Ceph packages require a detected Linux distribution".into())
    })?;
    ...
    match manager {
        Manager::Apt => {
            let suite = distribution.version_codename.as_deref().ok_or_else(|| {
                AdapterError::Unsupported("Ceph APT requires a distribution codename".into())
            })?;
            ...
            if !supported {
                return Err(AdapterError::Unsupported(format!(
                    "Ceph squid APT coverage excludes {} {}",
        ...
        Manager::Rpm => {
            ...
            {
                return Err(AdapterError::Unsupported(format!(
                    "Ceph squid RPM coverage is EL9 only, not {} {}",
...
fn mapped_uri(
    ...
    match (manager, provider) {
        (Manager::Apt, "aliyun" | "nju" | "tuna" | "ustc") => {
            Ok(format!("{endpoint}/debian-squid"))
        }
        (Manager::Apt, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed Ceph APT repository".into(),
        )),
        (Manager::Rpm, "aliyun" | "nju" | "tuna" | "ustc") => Ok(format!(
            "{endpoint}/rpm-squid/el9/{}",
            platform.architecture
        )),
        (Manager::Rpm, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed Ceph RPM repository".into(),
        )),
    }
}
...
fn platform_tag(platform: &Platform) -> Result<&'static str, AdapterError> {
    match (platform.family.as_str(), platform.release.as_str()) {
        ...
        _ => Err(AdapterError::Unsupported(
            "Ceph package tag is unavailable for this target".into(),
        )),
    }
}
...
fn rewrite_existing(
    ...
    let source = parse_repository(manager, text, path)?.ok_or_else(|| {
        AdapterError::InvalidConfiguration("selected Ceph repository disappeared".into())
    })?;
    if source.channel != channel {
        return Err(AdapterError::Unsupported(
            "Ceph repository channel changed during planning".into(),
        ));
    }
...
fn replace_active_once(
    ...
    if ranges.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Ceph repository URI in {} is missing or duplicated",
            path.display()
        )));
    }
...
fn selected_endpoint(selections: &[MirrorSelection]) -> Result<(String, String), AdapterError> {
    if selections.len() != 1
        || selections[0].tool_id != "ceph"
        || selections[0].upstream_id != UPSTREAM
    {
        return Err(AdapterError::InvalidConfiguration(
            "Ceph packages require exactly one repository selection".into(),
        ));
    }
    ...
    let role_url = |role| -> Result<String, AdapterError> {
        ...
        if endpoints.len() != 1 {
            return Err(AdapterError::InvalidConfiguration(format!(
                "Ceph package selection needs one HTTPS {role:?} endpoint"
            )));
        }
        normalized_endpoint(&endpoints[0].url).ok_or_else(|| {
            AdapterError::InvalidConfiguration("Ceph package endpoint is unsafe".into())
        })
    };
    let index = role_url(EndpointRole::Index)?;
    if role_url(EndpointRole::Metadata)? != index || role_url(EndpointRole::Packages)? != index {
        return Err(AdapterError::InvalidConfiguration(
            "Ceph package index, metadata, and package endpoints must match".into(),
        ));
    }
    ...
    {
        return Err(AdapterError::InvalidConfiguration(
            "Ceph package provider and endpoint are not reviewed".into(),
        ));
    }
...
fn validate_policy(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    for source in &current.sources {
        match metadata(source, "kind")? {
            "multiple-modern-repositories" => {
                return Err(AdapterError::InvalidConfiguration(
                    "multiple modern Ceph package repositories are active".into(),
                ));
            }
            "coverage-mismatch" => {
                return Err(AdapterError::Unsupported(
                    "Ceph repository distribution, release, or channel does not match the host"
                        .into(),
                ));
            }
            "keyring-missing" => {
                return Err(AdapterError::Unsupported(
                    "Ceph APT keyring is missing; repository security cannot be preserved".into(),
                ));
            }
            "gpgcheck-disabled" => {
                return Err(AdapterError::Unsupported(
                    "Ceph RPM repository has gpgcheck disabled".into(),
                ));
            }
            "gpgkey-missing" => {
                return Err(AdapterError::Unsupported(
                    "Ceph RPM repository has no GPG key configured".into(),
                ));
            }
            "managed-target-conflict" => {
                return Err(AdapterError::Unsupported(
                    "Ceph package target belongs to another MirrorSwitch format".into(),
                ));
            }
            _ => {}
        }
...
fn verify_apt(runtime: &dyn Runtime, target: &Path) -> Result<String, AdapterError> {
    ...
    command_output(runtime.run("apt-get", &update)?, "Ceph APT refresh")?;
    let mut query = Vec::new();
    for option in &options {
        query.extend(["-o".into(), option.clone()]);
    }
    query.extend(["policy".into(), "ceph".into()]);
    command_output(runtime.run("apt-cache", &query)?, "Ceph APT query")
}
...
fn verify_rpm(
    ...
    let repo_id = repo_id.unwrap_or("mirrorswitch-ceph");
    ...
    command_output(runtime.run("dnf", &refresh)?, "Ceph RPM refresh")?;
    ...
    command_output(runtime.run("dnf", &query)?, "Ceph RPM query")
}
...
fn find_document<'a>(
    ...
    if matches.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Ceph package state must contain one {format} document"
        )));
    }
    Ok(matches[0])
}
...
fn current_value(current: &CurrentConfiguration, kind: &str) -> Result<String, AdapterError> {
    ...
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Ceph package state has ambiguous {kind}"
        )));
    }
    Ok(values[0].clone())
}
...
fn snapshot_source(kind: &str, value: &str) -> ConfiguredSource {
    ConfiguredSource {
        upstream_id: None,
        url: format!("ceph-package-snapshot:{value}"),
        enabled: true,
        metadata: BTreeMap::from([
...
fn metadata<'a>(source: &'a ConfiguredSource, key: &str) -> Result<&'a str, AdapterError> {
    let values = source.metadata.get(key).ok_or_else(|| {
        AdapterError::InvalidConfiguration(format!("Ceph package source is missing {key} metadata"))
    })?;
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Ceph package source has ambiguous {key} metadata"
        )));
    }
    Ok(&values[0])
}
...
fn validate_path(path: &Path, kind: &str) -> Result<(), AdapterError> {
    ...
    {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Ceph package {kind} path {} is unsafe",
            path.display()
        )));
    }
    Ok(())
...
fn utf8<'a>(path: &Path, contents: &'a [u8]) -> Result<&'a str, AdapterError> {
    std::str::from_utf8(contents).map_err(|_| {
        AdapterError::InvalidConfiguration(format!(
            "Ceph package configuration {} is not UTF-8",
            path.display()
        ))
    })
}
