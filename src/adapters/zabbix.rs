...
const MANAGED_MARKER: &str = "# Managed by MirrorSwitch: Zabbix package repository v1";
const DEFAULT_KEYRING: &str = "/usr/share/keyrings/zabbix-official-repo.gpg";
const CHANNEL: &str = "7.4";
...
impl Adapter for ZabbixAdapter {
    ...
    fn detect(
            ...
            Some(version) => {
                    ...
                    .is_some_and(|configured| configured != channel)
                {
                    return Err(AdapterError::Unsupported(format!(
                        "installed Zabbix {version} does not match configured channel {}",
        ...
        if channel != CHANNEL {
            return Err(AdapterError::Unsupported(format!(
                "Zabbix channel {channel} is outside reviewed 7.4 LTS coverage"
            )));
        }
        ...
        if configured > 1 {
            return Err(AdapterError::InvalidConfiguration(
                "multiple Zabbix Community server repositories are active".into(),
            ));
        }
        ...
        Ok(Some(DetectedTool {
            ...
            evidence: vec![
                ...
                format!("target Zabbix channel is {channel}"),
                format!(
                    "installed Zabbix version is {}",
                    installed.as_deref().unwrap_or("not detected")
                ),
                format!("configured Zabbix Community server repositories: {configured}"),
                ...
                "Zabbix majors, release packages, server/agent packages, private repositories, and service data are separate surfaces".into(),
            ],
        }))
    }
    ...
    fn read_current(
        ...
        require_linux(context)?;
        require_scope(scope)?;
        if detected.tool_id != "zabbix" {
            return Err(AdapterError::InvalidConfiguration(
                "Zabbix package read received another tool's detection result".into(),
            ));
        }
        let manager = manager(runtime)?;
        ...
        let channel = match installed.as_deref() {
            ...
            None => configured_channel.clone().ok_or_else(|| {
                AdapterError::Unsupported(
                    "Zabbix channel cannot be inferred from the binary or repository".into(),
                )
            })?,
        };
        if channel != CHANNEL {
            ...
            .is_some_and(|configured| configured != channel)
        {
            return Err(AdapterError::Unsupported(
                "installed Zabbix version and configured server channel differ".into(),
            ));
        }
        let platform = platform(context, manager)?;
        ...
        if detected.version.as_deref() != Some(effective_version) {
            return Err(AdapterError::Conflict(
                "Zabbix package target version changed after detection".into(),
            ));
        }
        ...
        for document in observed {
                ...
                match manager {
                    Manager::Apt => {
                        let keyring = source.signed_by.as_deref().ok_or_else(|| {
                            AdapterError::Unsupported(
                                "modern Zabbix APT source has no explicit Signed-By keyring".into(),
                            )
                        })?;
            ...
            documents.push(ConfigurationDocument {
                ...
                format: if document.path == target {
                    "zabbix-package-target".into()
                } else {
                    "zabbix-package-read-only".into()
                },
    ...
    fn plan(
        ...
        let target = find_document(current, "zabbix-package-target")?;
        ...
        let changes = if target.contents == rendered {
            ...
            vec![PlannedFileChange {
                ...
                new_mode: None,
                summary: format!(
                    "map only the Zabbix {channel} {} package channel while preserving GPG, pinning, credentials, and unrelated repositories",
                    manager.name()
                ),
            }]
        };
        ...
        let result = (|| {
            ...
            if !receipt.changed_targets.contains(&physical) {
                return Err(AdapterError::Verification(
                    "Zabbix package transaction contains no known target".into(),
                ));
            }
            let selected = documents
                ...
                .ok_or_else(|| {
                    AdapterError::Verification(
                        "Zabbix package repository is not active after apply".into(),
                    )
                })?;
            ...
            if !normalized.contains("zabbix-server-pgsql")
                || !normalized.contains("zabbix-agent2")
                || !normalized.contains("7.4")
            {
                return Err(AdapterError::Verification(
                    "system package manager did not expose Zabbix 7.4 server and agent packages"
                        .into(),
                ));
            }
            Ok(VerificationResult {
                valid: true,
                summary: format!(
                    "{} refreshed and queried Zabbix server/agent {} through {}",
                    manager.name(),
                    selected.channel,
                    selected.uri
                ),
    ...
    fn restore(
        ...
        Ok(RestoreResult {
            ...
            summary: format!(
                "restored {} Zabbix package repository files from {}",
    ...
    fn from_name(value: &str) -> Result<Self, AdapterError> {
        match value {
            ...
            _ => Err(AdapterError::InvalidConfiguration(
                "unknown Zabbix package manager".into(),
            )),
        }
    }
}
...
fn require_linux(context: &SystemContext) -> Result<(), AdapterError> {
    if context.os != OperatingSystem::Linux {
        return Err(AdapterError::Unsupported(
            "Zabbix package adapter supports Linux only".into(),
        ));
    }
    Ok(())
}
...
fn require_scope(scope: ConfigurationScope) -> Result<(), AdapterError> {
    if scope != ConfigurationScope::System {
        return Err(AdapterError::Unsupported(
            "Zabbix package repositories require system scope".into(),
        ));
    }
    Ok(())
}
...
fn require_current(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    if current.tool_id != "zabbix" || current.scope != ConfigurationScope::System {
        return Err(AdapterError::InvalidConfiguration(
            "Zabbix package operation received another tool or scope".into(),
        ));
    }
    Ok(())
}
...
fn manager(runtime: &dyn Runtime) -> Result<Manager, AdapterError> {
    if runtime.command_exists("apt-get") && runtime.command_exists("apt-cache") {
        ...
        Err(AdapterError::Unsupported(
            "Zabbix packages require apt-get/apt-cache or dnf".into(),
        ))
    }
}
...
fn read_documents(
    ...
    let (main, directory, target, extensions) = match manager {
        Manager::Apt => (
            ...
            PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-zabbix.sources"),
            &["list", "sources"][..],
        ),
        Manager::Rpm => (
            None,
            PathBuf::from("/etc/yum.repos.d"),
            PathBuf::from("/etc/yum.repos.d/mirrorswitch-zabbix.repo"),
            &["repo"][..],
        ),
    };
    ...
    for stanza in text.split("\n\n") {
        ...
        for line in stanza.lines() {
            ...
            if let Some(value) = active.strip_prefix("URIs:") {
                for uri in value
                    .split_whitespace()
                    .filter(|uri| is_zabbix_apt_uri(uri))
                {
                    if deb822_uri
                        ...
                        .is_some()
                    {
                        return Err(AdapterError::InvalidConfiguration(format!(
                            "multiple Zabbix APT URIs exist in {}",
                            path.display()
                        )));
                    }
                }
            ...
            for (index, value) in fields.iter().enumerate() {
                if !is_zabbix_apt_uri(value) {
                    continue;
                }
                let suite = fields.get(index + 1).ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "Zabbix APT source in {} has no suite",
                        path.display()
                    ))
                })?;
                matches.push((
        ...
        if let Some(uri) = deb822_uri {
            matches.push((
                ...
                deb822_suite.ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "Zabbix deb822 source in {} has no suite",
                        path.display()
                    ))
                })?,
                deb822_components,
...
fn apt_identity(
    ...
    if components != ["main"] {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Zabbix APT source in {} must use the main component",
            path.display()
        )));
    }
    let after = uri
        ...
        .ok_or_else(|| {
            AdapterError::InvalidConfiguration(format!(
                "Zabbix APT source in {} has no versioned repo path",
                path.display()
            ))
        })?;
    let mut parts = after.split('/');
    let channel = parts.next().unwrap_or_default();
    let stability = parts.next().unwrap_or_default();
    let family = parts.next().unwrap_or_default();
    if version_pair(channel).is_none()
        || stability != "stable"
        || !matches!(family, "debian" | "ubuntu")
    {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Zabbix APT source in {} has an invalid version or family",
            path.display()
        )));
    }
...
fn parse_rpm_repository(text: &str, path: &Path) -> Result<Option<RepositorySource>, AdapterError> {
    ...
    let matches = values
        ...
        .filter_map(|(repo_id, values)| {
            ...
            (values.get("enabled").is_none_or(|value| value != "0") && is_zabbix_rpm_uri(uri))
                .then(|| (repo_id.clone(), uri.trim_end_matches('/').to_owned()))
        })
        .collect::<Vec<_>>();
    if matches.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "multiple Zabbix Community server RPM repositories exist in {}",
            path.display()
        )));
    }
    matches
        ...
        .map(|(repo_id, uri)| {
            ...
            let (channel, release) = rpm_identity(uri).ok_or_else(|| {
                AdapterError::InvalidConfiguration(format!(
                    "Zabbix RPM URI in {} has an unknown channel or release",
                    path.display()
                ))
            })?;
            Ok(RepositorySource {
...
fn is_zabbix_apt_uri(value: &str) -> bool {
    let value = value.trim_end_matches('/');
    (value.starts_with("https://repo.zabbix.com/zabbix/")
        || value.starts_with("http://repo.zabbix.com/zabbix/")
        || ACTIONABLE_ENDPOINTS
            .iter()
            .any(|(_, endpoint)| value.starts_with(&format!("{endpoint}/zabbix/"))))
        && value.contains("/stable/")
        && (value.ends_with("/debian") || value.ends_with("/ubuntu"))
}

fn is_zabbix_rpm_uri(value: &str) -> bool {
    let value = value.trim_end_matches('/');
    (value.starts_with("http://repo.zabbix.com/zabbix/")
        || value.starts_with("https://repo.zabbix.com/zabbix/")
        || ACTIONABLE_ENDPOINTS
            .iter()
            .any(|(_, endpoint)| value.starts_with(&format!("{endpoint}/zabbix/"))))
        && value.contains("/stable/rhel/9/")
}

fn rpm_identity(value: &str) -> Option<(String, String)> {
    let channel = value.rsplit_once("/zabbix/")?.1.split('/').next()?;
    (version_pair(channel).is_some() && value.contains("/stable/rhel/9/"))
...
fn unique_configured_channel(
    ...
    if values.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(
            "configured Zabbix repositories cross product channels".into(),
        ));
    }
...
fn choose_target(manager: Manager, documents: &[ObservedDocument]) -> PathBuf {
    documents
        ...
        .unwrap_or_else(|| match manager {
            Manager::Apt => PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-zabbix.sources"),
            Manager::Rpm => PathBuf::from("/etc/yum.repos.d/mirrorswitch-zabbix.repo"),
        })
}

fn is_managed_path(path: &Path) -> bool {
    path.file_name().is_some_and(|name| {
        name == "mirrorswitch-zabbix.sources" || name == "mirrorswitch-zabbix.repo"
    })
}

fn installed_version(runtime: &dyn Runtime) -> Result<Option<String>, AdapterError> {
    if !runtime.command_exists("zabbix_server") {
        return Ok(None);
    }
    let output = runtime.run("zabbix_server", &["--version".into()])?;
    if !output.status.success() {
        return Ok(None);
    }
    let output = String::from_utf8(output.stdout)
        .map_err(|_| AdapterError::Runtime("zabbix_server --version is not UTF-8".into()))?;
    Ok(output
        .split_whitespace()
...
fn channel_from_version(value: &str) -> Result<&'static str, AdapterError> {
    match version_pair(value) {
        ...
        Some((major, minor)) => Err(AdapterError::Unsupported(format!(
            "installed Zabbix {major}.{minor} is outside reviewed 7.4 stable coverage"
        ))),
        None => Err(AdapterError::Unsupported(format!(
            "installed Zabbix version {value} is invalid"
        ))),
    }
}
...
fn platform(context: &SystemContext, manager: Manager) -> Result<Platform, AdapterError> {
    let distribution = context.distribution.as_ref().ok_or_else(|| {
        AdapterError::Unsupported("Zabbix packages require a detected Linux distribution".into())
    })?;
    ...
    match manager {
        Manager::Apt => {
            let suite = distribution.version_codename.as_deref().ok_or_else(|| {
                AdapterError::Unsupported("Zabbix APT requires a distribution codename".into())
            })?;
            ...
            if !supported {
                return Err(AdapterError::Unsupported(format!(
                    "Zabbix 7.4 APT coverage excludes {} {}",
        ...
        Manager::Rpm => {
            ...
            {
                return Err(AdapterError::Unsupported(format!(
                    "Zabbix 7.4 RPM coverage is EL9 only, not {} {}",
...
fn mapped_uri(
    ...
    match (manager, provider) {
        (Manager::Apt, "aliyun" | "huaweicloud" | "nju") => {
            Ok(format!("{endpoint}/zabbix/7.4/stable/{}", platform.family))
        }
        (Manager::Apt, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed Zabbix APT repository".into(),
        )),
        (Manager::Rpm, "aliyun" | "huaweicloud" | "nju") => Ok(format!(
            "{endpoint}/zabbix/7.4/stable/rhel/{}/{}",
            platform.release, platform.architecture
        )),
        (Manager::Rpm, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed Zabbix RPM repository".into(),
        )),
    }
}
...
fn platform_tag(platform: &Platform) -> Result<&'static str, AdapterError> {
    match (platform.family.as_str(), platform.release.as_str()) {
        ("debian", "bookworm") => Ok("debian12"),
        ("ubuntu", "jammy") => Ok("ubuntu22.04"),
        ("ubuntu", "noble") => Ok("ubuntu24.04"),
        ("el", "9") => Ok("el9"),
        _ => Err(AdapterError::Unsupported(
            "Zabbix package tag is unavailable for this target".into(),
        )),
    }
}
...
fn rewrite_existing(
    ...
    let source = parse_repository(manager, text, path)?.ok_or_else(|| {
        AdapterError::InvalidConfiguration("selected Zabbix repository disappeared".into())
    })?;
    if source.channel != channel {
        return Err(AdapterError::Unsupported(
            "Zabbix repository channel changed during planning".into(),
        ));
    }
...
fn replace_active_once(
    ...
    if ranges.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Zabbix repository URI in {} is missing or duplicated",
            path.display()
        )));
    }
...
fn selected_endpoint(selections: &[MirrorSelection]) -> Result<(String, String), AdapterError> {
    if selections.len() != 1
        || selections[0].tool_id != "zabbix"
        || selections[0].upstream_id != UPSTREAM
    {
        return Err(AdapterError::InvalidConfiguration(
            "Zabbix packages require exactly one repository selection".into(),
        ));
    }
    ...
    let role_url = |role| -> Result<String, AdapterError> {
        ...
        if endpoints.len() != 1 {
            return Err(AdapterError::InvalidConfiguration(format!(
                "Zabbix package selection needs one HTTPS {role:?} endpoint"
            )));
        }
        normalized_endpoint(&endpoints[0].url).ok_or_else(|| {
            AdapterError::InvalidConfiguration("Zabbix package endpoint is unsafe".into())
        })
    };
    let index = role_url(EndpointRole::Index)?;
    if role_url(EndpointRole::Metadata)? != index || role_url(EndpointRole::Packages)? != index {
        return Err(AdapterError::InvalidConfiguration(
            "Zabbix package index, metadata, and package endpoints must match".into(),
        ));
    }
    ...
    {
        return Err(AdapterError::InvalidConfiguration(
            "Zabbix package provider and endpoint are not reviewed".into(),
        ));
    }
...
fn validate_policy(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    for source in &current.sources {
        match metadata(source, "kind")? {
            "multiple-modern-repositories" => {
                return Err(AdapterError::InvalidConfiguration(
                    "multiple modern Zabbix package repositories are active".into(),
                ));
            }
            "coverage-mismatch" => {
                return Err(AdapterError::Unsupported(
                    "Zabbix repository distribution, release, or channel does not match the host"
                        .into(),
                ));
            }
            "keyring-missing" => {
                return Err(AdapterError::Unsupported(
                    "Zabbix APT keyring is missing; repository security cannot be preserved".into(),
                ));
            }
            "gpgcheck-disabled" => {
                return Err(AdapterError::Unsupported(
                    "Zabbix RPM repository has gpgcheck disabled".into(),
                ));
            }
            "gpgkey-missing" => {
                return Err(AdapterError::Unsupported(
                    "Zabbix RPM repository has no GPG key configured".into(),
                ));
            }
            "managed-target-conflict" => {
                return Err(AdapterError::Unsupported(
                    "Zabbix package target belongs to another MirrorSwitch format".into(),
                ));
            }
            _ => {}
        }
...
fn verify_apt(runtime: &dyn Runtime, target: &Path) -> Result<String, AdapterError> {
    ...
    command_output(runtime.run("apt-get", &update)?, "Zabbix APT refresh")?;
    ...
    query.extend([
        "policy".into(),
        "zabbix-server-pgsql".into(),
        "zabbix-agent2".into(),
    ]);
    command_output(runtime.run("apt-cache", &query)?, "Zabbix APT query")
}
...
fn verify_rpm(
    ...
    let repo_id = repo_id.unwrap_or("mirrorswitch-zabbix");
    ...
    command_output(runtime.run("dnf", &refresh)?, "Zabbix RPM refresh")?;
    ...
    command_output(runtime.run("dnf", &query)?, "Zabbix RPM query")
}
...
fn find_document<'a>(
    ...
    if matches.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Zabbix package state must contain one {format} document"
        )));
    }
    Ok(matches[0])
}
...
fn current_value(current: &CurrentConfiguration, kind: &str) -> Result<String, AdapterError> {
    ...
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Zabbix package state has ambiguous {kind}"
        )));
    }
    Ok(values[0].clone())
}
...
fn snapshot_source(kind: &str, value: &str) -> ConfiguredSource {
    ConfiguredSource {
        upstream_id: None,
        url: format!("zabbix-package-snapshot:{value}"),
        enabled: true,
        metadata: BTreeMap::from([
...
fn metadata<'a>(source: &'a ConfiguredSource, key: &str) -> Result<&'a str, AdapterError> {
    let values = source.metadata.get(key).ok_or_else(|| {
        AdapterError::InvalidConfiguration(format!(
            "Zabbix package source is missing {key} metadata"
        ))
    })?;
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Zabbix package source has ambiguous {key} metadata"
        )));
    }
    Ok(&values[0])
}
...
fn validate_path(path: &Path, kind: &str) -> Result<(), AdapterError> {
    ...
    {
        return Err(AdapterError::InvalidConfiguration(format!(
            "Zabbix package {kind} path {} is unsafe",
            path.display()
        )));
    }
    Ok(())
...
fn utf8<'a>(path: &Path, contents: &'a [u8]) -> Result<&'a str, AdapterError> {
    std::str::from_utf8(contents).map_err(|_| {
        AdapterError::InvalidConfiguration(format!(
            "Zabbix package configuration {} is not UTF-8",
            path.display()
        ))
    })
}
