...
const MANAGED_MARKER: &str = "# Managed by MirrorSwitch: MySQL package repository v1";
const DEFAULT_KEYRING: &str = "/usr/share/keyrings/mysql-apt-config.gpg";
const CHANNEL: &str = "8.4-lts";
...
impl Adapter for MySqlAdapter {
    ...
    fn detect(
            ...
            Some(version) => {
                    ...
                    .is_some_and(|configured| configured != channel)
                {
                    return Err(AdapterError::Unsupported(format!(
                        "installed MySQL {version} does not match configured channel {}",
        ...
        if channel != CHANNEL {
            return Err(AdapterError::Unsupported(format!(
                "MySQL channel {channel} is outside reviewed 8.4 LTS coverage"
            )));
        }
        let platform = platform(context, manager)?;
        if manager == Manager::Apt && context.architecture == Architecture::Arm64 {
            return Err(AdapterError::Unsupported(
                "MySQL Community APT publishes amd64 but no arm64 packages".into(),
            ));
        }
        ...
        if configured > 1 {
            return Err(AdapterError::InvalidConfiguration(
                "multiple MySQL Community server repositories are active".into(),
            ));
        }
        ...
        Ok(Some(DetectedTool {
            ...
            evidence: vec![
                ...
                format!("target MySQL channel is {channel}"),
                format!(
                    "installed MySQL version is {}",
                    installed.as_deref().unwrap_or("not detected")
                ),
                format!("configured MySQL Community server repositories: {configured}"),
                ...
                "MySQL tools/connectors, MariaDB, private repositories, and service data are separate surfaces".into(),
            ],
        }))
    }
    ...
    fn read_current(
        ...
        require_linux(context)?;
        require_scope(scope)?;
        if detected.tool_id != "mysql" {
            return Err(AdapterError::InvalidConfiguration(
                "MySQL package read received another tool's detection result".into(),
            ));
        }
        let manager = manager(runtime)?;
        ...
        let channel = match installed.as_deref() {
            ...
            None => configured_channel.clone().ok_or_else(|| {
                AdapterError::Unsupported(
                    "MySQL channel cannot be inferred from the binary or repository".into(),
                )
            })?,
        };
        if channel != CHANNEL {
        ...
        {
            return Err(AdapterError::Unsupported(
                "installed MySQL version and configured server channel differ".into(),
            ));
        }
        let platform = platform(context, manager)?;
        if manager == Manager::Apt && context.architecture == Architecture::Arm64 {
        ...
        if detected.version.as_deref() != Some(effective_version) {
            return Err(AdapterError::Conflict(
                "MySQL package target version changed after detection".into(),
            ));
        }
        ...
        for document in observed {
                ...
                match manager {
                    Manager::Apt => {
                        let keyring = source.signed_by.as_deref().ok_or_else(|| {
                            AdapterError::Unsupported(
                                "modern MySQL APT source has no explicit Signed-By keyring".into(),
                            )
                        })?;
            ...
            documents.push(ConfigurationDocument {
                ...
                format: if document.path == target {
                    "mysql-package-target".into()
                } else {
                    "mysql-package-read-only".into()
                },
    ...
    fn plan(
        ...
        let target = find_document(current, "mysql-package-target")?;
        ...
        let changes = if target.contents == rendered {
            ...
            vec![PlannedFileChange {
                ...
                new_mode: None,
                summary: format!(
                    "map only the MySQL {channel} {} package channel while preserving GPG, pinning, credentials, and unrelated repositories",
                    manager.name()
                ),
            }]
        };
        ...
        let result = (|| {
            ...
            if !receipt.changed_targets.contains(&physical) {
                return Err(AdapterError::Verification(
                    "MySQL package transaction contains no known target".into(),
                ));
            }
            let selected = documents
                ...
                .ok_or_else(|| {
                    AdapterError::Verification(
                        "MySQL package repository is not active after apply".into(),
                    )
                })?;
            ...
            if !output.contains("mysql-community-server") || !output.contains("8.4") {
                return Err(AdapterError::Verification(
                    "system package manager did not expose mysql-community-server 8.4".into(),
                ));
            }
            Ok(VerificationResult {
                valid: true,
                summary: format!(
                    "{} refreshed and queried mysql-community-server from MySQL {} through {}",
                    manager.name(),
                    selected.channel,
                    selected.uri
                ),
    ...
    fn restore(
        ...
        Ok(RestoreResult {
            ...
            summary: format!(
                "restored {} MySQL package repository files from {}",
    ...
    fn from_name(value: &str) -> Result<Self, AdapterError> {
        match value {
            ...
            _ => Err(AdapterError::InvalidConfiguration(
                "unknown MySQL package manager".into(),
            )),
        }
    }
}
...
fn require_linux(context: &SystemContext) -> Result<(), AdapterError> {
    if context.os != OperatingSystem::Linux {
        return Err(AdapterError::Unsupported(
            "MySQL package adapter supports Linux only".into(),
        ));
    }
    Ok(())
}
...
fn require_scope(scope: ConfigurationScope) -> Result<(), AdapterError> {
    if scope != ConfigurationScope::System {
        return Err(AdapterError::Unsupported(
            "MySQL package repositories require system scope".into(),
        ));
    }
    Ok(())
}
...
fn require_current(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    if current.tool_id != "mysql" || current.scope != ConfigurationScope::System {
        return Err(AdapterError::InvalidConfiguration(
            "MySQL package operation received another tool or scope".into(),
        ));
    }
    Ok(())
}
...
fn manager(runtime: &dyn Runtime) -> Result<Manager, AdapterError> {
    if runtime.command_exists("apt-get") && runtime.command_exists("apt-cache") {
        ...
        Err(AdapterError::Unsupported(
            "MySQL packages require apt-get/apt-cache or dnf".into(),
        ))
    }
}
...
fn read_documents(
    ...
    let (main, directory, target, extensions) = match manager {
        Manager::Apt => (
            ...
            PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-mysql.sources"),
            &["list", "sources"][..],
        ),
        Manager::Rpm => (
            None,
            PathBuf::from("/etc/yum.repos.d"),
            PathBuf::from("/etc/yum.repos.d/mirrorswitch-mysql.repo"),
            &["repo"][..],
        ),
    };
    ...
    for stanza in text.split("\n\n") {
        ...
        for line in stanza.lines() {
            ...
            if let Some(value) = active.strip_prefix("URIs:") {
                for uri in value.split_whitespace().filter(|uri| is_mysql_apt_uri(uri)) {
                    if deb822_uri
                        .replace(uri.trim_end_matches('/').into())
                        .is_some()
                    {
                        return Err(AdapterError::InvalidConfiguration(format!(
                            "multiple MySQL APT URIs exist in {}",
                            path.display()
                        )));
                    }
                }
            ...
            for (index, value) in fields.iter().enumerate() {
                if !is_mysql_apt_uri(value) {
                    continue;
                }
                let suite = fields.get(index + 1).ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "MySQL APT source in {} has no suite",
                        path.display()
                    ))
                })?;
                matches.push((
        ...
        if let Some(uri) = deb822_uri {
            matches.push((
                ...
                deb822_suite.ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "MySQL deb822 source in {} has no suite",
                        path.display()
                    ))
                })?,
                deb822_components,
    ...
    matches
        ...
        .map(|(uri, suite, components, signed_by)| {
            let family = apt_family(uri, suite).ok_or_else(|| {
                AdapterError::InvalidConfiguration(format!(
                    "MySQL APT source in {} has an unknown distribution family",
                    path.display()
                ))
            })?;
            Ok(RepositorySource {
...
fn apt_channel(components: &[String], path: &Path) -> Result<String, AdapterError> {
    let channels = components
        ...
        .filter(|component| {
            matches!(
                component.as_str(),
                "mysql-8.0" | "mysql-8.4-lts" | "mysql-9.7-lts" | "mysql-innovation"
            )
        })
        .cloned()
        .collect::<BTreeSet<_>>();
    if channels.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MySQL APT source in {} must select exactly one Community server channel",
            path.display()
        )));
    }
    Ok(channels
...
fn apt_family<'a>(uri: &str, suite: &'a str) -> Option<&'a str> {
    if uri.ends_with("/ubuntu") {
        Some("ubuntu")
    } else if uri.ends_with("/debian") || suite == "bookworm" {
        Some("debian")
    } else {
        None
    }
...
fn parse_rpm_repository(text: &str, path: &Path) -> Result<Option<RepositorySource>, AdapterError> {
    ...
    let matches = values
        ...
        .filter_map(|(repo_id, values)| {
            ...
            (values.get("enabled").is_none_or(|value| value != "0") && is_mysql_rpm_uri(uri))
                .then(|| (repo_id.clone(), uri.trim_end_matches('/').to_owned()))
        })
        .collect::<Vec<_>>();
    if matches.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "multiple MySQL Community server RPM repositories exist in {}",
            path.display()
        )));
    }
    matches
        ...
        .map(|(repo_id, uri)| {
            ...
            let (channel, release) = rpm_identity(uri).ok_or_else(|| {
                AdapterError::InvalidConfiguration(format!(
                    "MySQL RPM URI in {} has an unknown channel or release",
                    path.display()
                ))
            })?;
            Ok(RepositorySource {
...
fn is_mysql_apt_uri(value: &str) -> bool {
    [
        "http://repo.mysql.com/apt/debian",
        "https://repo.mysql.com/apt/debian",
        "http://repo.mysql.com/apt/ubuntu",
        "https://repo.mysql.com/apt/ubuntu",
        "https://mirrors.nju.edu.cn/mysql/apt",
        "https://mirrors.tuna.tsinghua.edu.cn/mysql/apt/debian",
        "https://mirrors.tuna.tsinghua.edu.cn/mysql/apt/ubuntu",
        "https://mirrors.ustc.edu.cn/mysql-repo/apt/debian",
        "https://mirrors.ustc.edu.cn/mysql-repo/apt/ubuntu",
    ]
    .contains(&value.trim_end_matches('/'))
}

fn is_mysql_rpm_uri(value: &str) -> bool {
    let value = value.trim_end_matches('/');
    (value.starts_with("http://repo.mysql.com/yum/")
        || value.starts_with("https://repo.mysql.com/yum/")
        || ACTIONABLE_ENDPOINTS
            .iter()
            .any(|(_, endpoint)| value.starts_with(&format!("{endpoint}/yum/"))))
        && value.contains("mysql-8.4-community")
}

fn rpm_identity(value: &str) -> Option<(&'static str, &'static str)> {
    (value.contains("mysql-8.4-community") && (value.contains("/el/9/") || value.contains("-el9-")))
        .then_some((CHANNEL, "9"))
}
...
fn unique_configured_channel(
    ...
    if values.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(
            "configured MySQL repositories cross product channels".into(),
        ));
    }
...
fn choose_target(manager: Manager, documents: &[ObservedDocument]) -> PathBuf {
    documents
        ...
        .unwrap_or_else(|| match manager {
            Manager::Apt => PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-mysql.sources"),
            Manager::Rpm => PathBuf::from("/etc/yum.repos.d/mirrorswitch-mysql.repo"),
        })
}

fn is_managed_path(path: &Path) -> bool {
    path.file_name().is_some_and(|name| {
        name == "mirrorswitch-mysql.sources" || name == "mirrorswitch-mysql.repo"
    })
}

fn installed_version(runtime: &dyn Runtime) -> Result<Option<String>, AdapterError> {
    if !runtime.command_exists("mysql") {
        return Ok(None);
    }
    let output = runtime.run("mysql", &["--version".into()])?;
    if !output.status.success() {
        return Ok(None);
    }
    let output = String::from_utf8(output.stdout)
        .map_err(|_| AdapterError::Runtime("mysql --version is not UTF-8".into()))?;
    Ok(output
        .split_whitespace()
...
fn channel_from_version(value: &str) -> Result<&'static str, AdapterError> {
    match version_pair(value) {
        ...
        Some((major, minor)) => Err(AdapterError::Unsupported(format!(
            "installed MySQL {major}.{minor} is outside reviewed 8.4 LTS coverage"
        ))),
        None => Err(AdapterError::Unsupported(format!(
            "installed MySQL version {value} is invalid"
        ))),
    }
}
...
fn platform(context: &SystemContext, manager: Manager) -> Result<Platform, AdapterError> {
    let distribution = context.distribution.as_ref().ok_or_else(|| {
        AdapterError::Unsupported("MySQL packages require a detected Linux distribution".into())
    })?;
    ...
    match manager {
        Manager::Apt => {
            let suite = distribution.version_codename.as_deref().ok_or_else(|| {
                AdapterError::Unsupported("MySQL APT requires a distribution codename".into())
            })?;
            ...
            if !supported {
                return Err(AdapterError::Unsupported(format!(
                    "MySQL 8.4 APT coverage excludes {} {}",
        ...
        Manager::Rpm => {
            ...
            {
                return Err(AdapterError::Unsupported(format!(
                    "MySQL 8.4 RPM coverage is EL9 only, not {} {}",
...
fn mapped_uri(
    ...
    match (manager, provider) {
        (Manager::Apt, "tuna" | "ustc") => Ok(format!("{endpoint}/apt/{}", platform.family)),
        (Manager::Apt, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed current MySQL APT repository".into(),
        )),
        (Manager::Rpm, "tuna") => Ok(format!(
            "{endpoint}/yum/mysql-8.4-community-el{}-{}",
            platform.release, platform.architecture
        )),
        (Manager::Rpm, "nju" | "ustc") => Ok(format!(
            "{endpoint}/yum/mysql-8.4-community/el/{}/{}",
            platform.release, platform.architecture
        )),
        (Manager::Rpm, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed MySQL RPM repository".into(),
        )),
    }
}
...
fn representative_package_name(
    ...
    match (
        ...
        (Manager::Apt, "debian", "bookworm", "amd64") => {
            Ok("mysql-community-server_8.4.11-1debian12_amd64.deb")
        }
        (Manager::Apt, "ubuntu", "jammy", "amd64") => {
            Ok("mysql-community-server_8.4.11-1ubuntu22.04_amd64.deb")
        }
        (Manager::Apt, "ubuntu", "noble", "amd64") => {
            Ok("mysql-community-server_8.4.11-1ubuntu24.04_amd64.deb")
        }
        (Manager::Rpm, "el", "9", "x86_64") => Ok("mysql-community-server-8.4.0-1.el9.x86_64.rpm"),
        (Manager::Rpm, "el", "9", "aarch64") => {
            Ok("mysql-community-server-8.4.0-1.el9.aarch64.rpm")
        }
        _ => Err(AdapterError::Unsupported(
            "MySQL representative package is unavailable for this target".into(),
        )),
    }
}
...
fn rewrite_existing(
    ...
    let source = parse_repository(manager, text, path)?.ok_or_else(|| {
        AdapterError::InvalidConfiguration("selected MySQL repository disappeared".into())
    })?;
    if source.channel != channel {
        return Err(AdapterError::Unsupported(
            "MySQL repository channel changed during planning".into(),
        ));
    }
...
fn replace_active_once(
    ...
    if ranges.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MySQL repository URI in {} is missing or duplicated",
            path.display()
        )));
    }
...
fn selected_endpoint(selections: &[MirrorSelection]) -> Result<(String, String), AdapterError> {
    if selections.len() != 1
        || selections[0].tool_id != "mysql"
        || selections[0].upstream_id != UPSTREAM
    {
        return Err(AdapterError::InvalidConfiguration(
            "MySQL packages require exactly one repository selection".into(),
        ));
    }
    ...
    let role_url = |role| -> Result<String, AdapterError> {
        ...
        if endpoints.len() != 1 {
            return Err(AdapterError::InvalidConfiguration(format!(
                "MySQL package selection needs one HTTPS {role:?} endpoint"
            )));
        }
        normalized_endpoint(&endpoints[0].url).ok_or_else(|| {
            AdapterError::InvalidConfiguration("MySQL package endpoint is unsafe".into())
        })
    };
    let index = role_url(EndpointRole::Index)?;
    if role_url(EndpointRole::Metadata)? != index || role_url(EndpointRole::Packages)? != index {
        return Err(AdapterError::InvalidConfiguration(
            "MySQL package index, metadata, and package endpoints must match".into(),
        ));
    }
    ...
    {
        return Err(AdapterError::InvalidConfiguration(
            "MySQL package provider and endpoint are not reviewed".into(),
        ));
    }
...
fn validate_policy(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    for source in &current.sources {
        match metadata(source, "kind")? {
            "multiple-modern-repositories" => {
                return Err(AdapterError::InvalidConfiguration(
                    "multiple modern MySQL package repositories are active".into(),
                ));
            }
            "coverage-mismatch" => {
                return Err(AdapterError::Unsupported(
                    "MySQL repository distribution, release, or channel does not match the host"
                        .into(),
                ));
            }
            "keyring-missing" => {
                return Err(AdapterError::Unsupported(
                    "MySQL APT keyring is missing; repository security cannot be preserved".into(),
                ));
            }
            "gpgcheck-disabled" => {
                return Err(AdapterError::Unsupported(
                    "MySQL RPM repository has gpgcheck disabled".into(),
                ));
            }
            "gpgkey-missing" => {
                return Err(AdapterError::Unsupported(
                    "MySQL RPM repository has no GPG key configured".into(),
                ));
            }
            "managed-target-conflict" => {
                return Err(AdapterError::Unsupported(
                    "MySQL package target belongs to another MirrorSwitch format".into(),
                ));
            }
            _ => {}
        }
...
fn verify_apt(runtime: &dyn Runtime, target: &Path) -> Result<String, AdapterError> {
    ...
    command_output(runtime.run("apt-get", &update)?, "MySQL APT refresh")?;
    let mut query = Vec::new();
    for option in &options {
        query.extend(["-o".into(), option.clone()]);
    }
    query.extend(["policy".into(), "mysql-community-server".into()]);
    command_output(runtime.run("apt-cache", &query)?, "MySQL APT query")
}
...
fn verify_rpm(
    ...
    let repo_id = repo_id.unwrap_or("mirrorswitch-mysql");
    ...
    command_output(runtime.run("dnf", &refresh)?, "MySQL RPM refresh")?;
    let mut query = common.to_vec();
    query.extend([
        "-q".into(),
        "list".into(),
        "--showduplicates".into(),
        "mysql-community-server".into(),
    ]);
    command_output(runtime.run("dnf", &query)?, "MySQL RPM query")
}
...
fn find_document<'a>(
    ...
    if matches.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MySQL package state must contain one {format} document"
        )));
    }
    Ok(matches[0])
}
...
fn current_value(current: &CurrentConfiguration, kind: &str) -> Result<String, AdapterError> {
    ...
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MySQL package state has ambiguous {kind}"
        )));
    }
    Ok(values[0].clone())
}
...
fn snapshot_source(kind: &str, value: &str) -> ConfiguredSource {
    ConfiguredSource {
        upstream_id: None,
        url: format!("mysql-package-snapshot:{value}"),
        enabled: true,
        metadata: BTreeMap::from([
...
fn metadata<'a>(source: &'a ConfiguredSource, key: &str) -> Result<&'a str, AdapterError> {
    let values = source.metadata.get(key).ok_or_else(|| {
        AdapterError::InvalidConfiguration(format!(
            "MySQL package source is missing {key} metadata"
        ))
    })?;
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MySQL package source has ambiguous {key} metadata"
        )));
    }
    Ok(&values[0])
}
...
fn validate_path(path: &Path, kind: &str) -> Result<(), AdapterError> {
    ...
    {
        return Err(AdapterError::InvalidConfiguration(format!(
            "MySQL package {kind} path {} is unsafe",
            path.display()
        )));
    }
    Ok(())
...
fn utf8<'a>(path: &Path, contents: &'a [u8]) -> Result<&'a str, AdapterError> {
    std::str::from_utf8(contents).map_err(|_| {
        AdapterError::InvalidConfiguration(format!(
            "MySQL package configuration {} is not UTF-8",
            path.display()
        ))
    })
}
