...
const MANAGED_MARKER: &str = "# Managed by MirrorSwitch: InfluxDB package repository v1";
const DEFAULT_KEYRING: &str = "/usr/share/keyrings/influxdata-archive-keyring.gpg";
...
impl Adapter for InfluxDbAdapter {
    ...
    fn detect(
            ...
            Some(version) => {
                    ...
                    .is_some_and(|configured| configured != channel)
                {
                    return Err(AdapterError::Unsupported(format!(
                        "installed InfluxDB {version} does not match configured channel {}",
        ...
        if channel != CHANNEL {
            return Err(AdapterError::Unsupported(format!(
                "InfluxDB channel {channel} is outside reviewed stable 2.x coverage"
            )));
        }
        ...
        if configured > 1 {
            return Err(AdapterError::InvalidConfiguration(
                "multiple InfluxDB Community server repositories are active".into(),
            ));
        }
        ...
        Ok(Some(DetectedTool {
            ...
            evidence: vec![
                ...
                format!("target InfluxDB channel is {channel}"),
                format!(
                    "installed InfluxDB version is {}",
                    installed.as_deref().unwrap_or("not detected")
                ),
                format!("configured InfluxDB Community server repositories: {configured}"),
                ...
                "InfluxDB 1.x, nightly, private repositories, and service data are separate surfaces".into(),
            ],
        }))
    }
    ...
    fn read_current(
        ...
        require_linux(context)?;
        require_scope(scope)?;
        if detected.tool_id != "influxdb" {
            return Err(AdapterError::InvalidConfiguration(
                "InfluxDB package read received another tool's detection result".into(),
            ));
        }
        let manager = manager(runtime)?;
        ...
        let channel = match installed.as_deref() {
            ...
            None => configured_channel.clone().ok_or_else(|| {
                AdapterError::Unsupported(
                    "InfluxDB channel cannot be inferred from the binary or repository".into(),
                )
            })?,
        };
        if channel != CHANNEL {
            ...
            .is_some_and(|configured| configured != channel)
        {
            return Err(AdapterError::Unsupported(
                "installed InfluxDB version and configured server channel differ".into(),
            ));
        }
        let platform = platform(context, manager)?;
        ...
        if detected.version.as_deref() != Some(effective_version) {
            return Err(AdapterError::Conflict(
                "InfluxDB package target version changed after detection".into(),
            ));
        }
        ...
        for document in observed {
                ...
                match manager {
                    Manager::Apt => {
                        let keyring = source.signed_by.as_deref().ok_or_else(|| {
                            AdapterError::Unsupported(
                                "modern InfluxDB APT source has no explicit Signed-By keyring"
                                    .into(),
                            )
                        })?;
            ...
            documents.push(ConfigurationDocument {
                ...
                format: if document.path == target {
                    "influxdb-package-target".into()
                } else {
                    "influxdb-package-read-only".into()
                },
    ...
    fn plan(
        ...
        let target = find_document(current, "influxdb-package-target")?;
        ...
        let changes = if target.contents == rendered {
            ...
            vec![PlannedFileChange {
                ...
                new_mode: None,
                summary: format!(
                    "map only the InfluxDB {channel} {} package channel while preserving GPG, pinning, credentials, and unrelated repositories",
                    manager.name()
                ),
            }]
        };
        ...
        let result = (|| {
            ...
            if !receipt.changed_targets.contains(&physical) {
                return Err(AdapterError::Verification(
                    "InfluxDB package transaction contains no known target".into(),
                ));
            }
            let selected = documents
                ...
                .ok_or_else(|| {
                    AdapterError::Verification(
                        "InfluxDB package repository is not active after apply".into(),
                    )
                })?;
            ...
            if !output.contains("influxdb2") || !output.contains("2.") {
                return Err(AdapterError::Verification(
                    "system package manager did not expose InfluxDB 2.x server packages".into(),
                ));
            }
            Ok(VerificationResult {
                valid: true,
                summary: format!(
                    "{} refreshed and queried influxdb2 from InfluxDB {} through {}",
                    manager.name(),
                    selected.channel,
                    selected.uri
                ),
    ...
    fn restore(
        ...
        Ok(RestoreResult {
            ...
            summary: format!(
                "restored {} InfluxDB package repository files from {}",
    ...
    fn from_name(value: &str) -> Result<Self, AdapterError> {
        match value {
            ...
            _ => Err(AdapterError::InvalidConfiguration(
                "unknown InfluxDB package manager".into(),
            )),
        }
    }
}
...
impl Platform {
    fn repository_version(&self, _channel: &str) -> String {
        if self.family == "el" {
            format!("el-{}-stable-2-{}", self.release, self.architecture)
        } else {
            format!("{}-stable-2-{}", self.family, self.architecture)
        }
    }
}
...
fn require_linux(context: &SystemContext) -> Result<(), AdapterError> {
    if context.os != OperatingSystem::Linux {
        return Err(AdapterError::Unsupported(
            "InfluxDB package adapter supports Linux only".into(),
        ));
    }
    Ok(())
}
...
fn require_scope(scope: ConfigurationScope) -> Result<(), AdapterError> {
    if scope != ConfigurationScope::System {
        return Err(AdapterError::Unsupported(
            "InfluxDB package repositories require system scope".into(),
        ));
    }
    Ok(())
}
...
fn require_current(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    if current.tool_id != "influxdb" || current.scope != ConfigurationScope::System {
        return Err(AdapterError::InvalidConfiguration(
            "InfluxDB package operation received another tool or scope".into(),
        ));
    }
    Ok(())
}
...
fn manager(runtime: &dyn Runtime) -> Result<Manager, AdapterError> {
    if runtime.command_exists("apt-get") && runtime.command_exists("apt-cache") {
        ...
        Err(AdapterError::Unsupported(
            "InfluxDB packages require apt-get/apt-cache or dnf".into(),
        ))
    }
}
...
fn read_documents(
    ...
    let (main, directory, target, extensions) = match manager {
        Manager::Apt => (
            ...
            PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-influxdb.sources"),
            &["list", "sources"][..],
        ),
        Manager::Rpm => (
            None,
            PathBuf::from("/etc/yum.repos.d"),
            PathBuf::from("/etc/yum.repos.d/mirrorswitch-influxdb.repo"),
            &["repo"][..],
        ),
    };
...
fn parse_apt_repository(text: &str, path: &Path) -> Result<Option<RepositorySource>, AdapterError> {
    ...
    for stanza in text.split("\n\n") {
        ...
        for line in stanza.lines() {
            ...
            if let Some(value) = active.strip_prefix("URIs:") {
                for uri in value
                    .split_whitespace()
                    .filter(|uri| is_influxdb_apt_uri(uri))
                {
                    if deb822_uri
                        ...
                        .is_some()
                    {
                        return Err(AdapterError::InvalidConfiguration(format!(
                            "multiple InfluxDB APT URIs exist in {}",
                            path.display()
                        )));
                    }
                }
            ...
            for (index, value) in fields.iter().enumerate() {
                if !is_influxdb_apt_uri(value) {
                    continue;
                }
                let suite = fields.get(index + 1).ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "InfluxDB APT source in {} has no suite",
                        path.display()
                    ))
                })?;
                matches.push((
        ...
        if let Some(uri) = deb822_uri {
            matches.push((
                ...
                deb822_suite.ok_or_else(|| {
                    AdapterError::InvalidConfiguration(format!(
                        "InfluxDB deb822 source in {} has no suite",
                        path.display()
                    ))
                })?,
                deb822_components,
    ...
    matches
        ...
        .map(|(uri, suite, components, signed_by)| {
            let family = apt_family(uri, suite).ok_or_else(|| {
                AdapterError::InvalidConfiguration(format!(
                    "InfluxDB APT source in {} has an unknown distribution family",
                    path.display()
                ))
            })?;
            Ok(RepositorySource {
...
fn apt_channel(components: &[String], path: &Path) -> Result<String, AdapterError> {
    if components != ["main"] {
        return Err(AdapterError::InvalidConfiguration(format!(
            "InfluxDB APT source in {} must use the main component",
            path.display()
        )));
    }
    Ok(CHANNEL.into())
...
fn apt_family<'a>(uri: &str, suite: &'a str) -> Option<&'a str> {
    if suite != CHANNEL {
        None
    } else if uri.ends_with("/ubuntu") {
        Some("ubuntu")
...
fn parse_rpm_repository(text: &str, path: &Path) -> Result<Option<RepositorySource>, AdapterError> {
    ...
    let matches = values
        ...
        .filter_map(|(repo_id, values)| {
            ...
            (values.get("enabled").is_none_or(|value| value != "0") && is_influxdb_rpm_uri(uri))
                .then(|| (repo_id.clone(), uri.trim_end_matches('/').to_owned()))
        })
        .collect::<Vec<_>>();
    if matches.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "multiple InfluxDB Community server RPM repositories exist in {}",
            path.display()
        )));
    }
    matches
        ...
        .map(|(repo_id, uri)| {
            ...
            let (channel, release) = rpm_identity(uri).ok_or_else(|| {
                AdapterError::InvalidConfiguration(format!(
                    "InfluxDB RPM URI in {} has an unknown channel or release",
                    path.display()
                ))
            })?;
            Ok(RepositorySource {
...
fn is_influxdb_apt_uri(value: &str) -> bool {
    [
        "http://repos.influxdata.com/debian",
        "https://repos.influxdata.com/debian",
        "http://repos.influxdata.com/ubuntu",
        "https://repos.influxdata.com/ubuntu",
        "https://mirrors.nju.edu.cn/influxdata/debian",
        "https://mirrors.nju.edu.cn/influxdata/ubuntu",
        "https://mirrors.tuna.tsinghua.edu.cn/influxdata/debian",
        "https://mirrors.tuna.tsinghua.edu.cn/influxdata/ubuntu",
        "https://mirrors.ustc.edu.cn/influxdata/debian",
    ]
    .contains(&value.trim_end_matches('/'))
}

fn is_influxdb_rpm_uri(value: &str) -> bool {
    let value = value.trim_end_matches('/');
    (value.starts_with("http://repos.influxdata.com/")
        || value.starts_with("https://repos.influxdata.com/")
        || ACTIONABLE_ENDPOINTS
            .iter()
            .any(|(_, endpoint)| value.starts_with(&format!("{endpoint}/yum/"))))
        && ((value.contains("/rhel/9/") && value.ends_with("stable"))
            || value.ends_with("/yum/el9-x86_64"))
}

fn rpm_identity(value: &str) -> Option<(&'static str, &'static str)> {
    ((value.contains("/rhel/9/") && value.trim_end_matches('/').ends_with("stable"))
        || value.trim_end_matches('/').ends_with("/yum/el9-x86_64"))
    .then_some((CHANNEL, "9"))
}
...
fn unique_configured_channel(
    ...
    if values.len() > 1 {
        return Err(AdapterError::InvalidConfiguration(
            "configured InfluxDB repositories cross product channels".into(),
        ));
    }
...
fn choose_target(manager: Manager, documents: &[ObservedDocument]) -> PathBuf {
    documents
        ...
        .unwrap_or_else(|| match manager {
            Manager::Apt => PathBuf::from("/etc/apt/sources.list.d/mirrorswitch-influxdb.sources"),
            Manager::Rpm => PathBuf::from("/etc/yum.repos.d/mirrorswitch-influxdb.repo"),
        })
}

fn is_managed_path(path: &Path) -> bool {
    path.file_name().is_some_and(|name| {
        name == "mirrorswitch-influxdb.sources" || name == "mirrorswitch-influxdb.repo"
    })
}

fn installed_version(runtime: &dyn Runtime) -> Result<Option<String>, AdapterError> {
    if !runtime.command_exists("influxd") {
        return Ok(None);
    }
    let output = runtime.run("influxd", &["version".into()])?;
    if !output.status.success() {
        return Ok(None);
    }
    let output = String::from_utf8(output.stdout)
        .map_err(|_| AdapterError::Runtime("influxd version is not UTF-8".into()))?;
    Ok(output
        .split_whitespace()
...
fn channel_from_version(value: &str) -> Result<&'static str, AdapterError> {
    match version_pair(value) {
        ...
        Some((major, minor)) => Err(AdapterError::Unsupported(format!(
            "installed InfluxDB {major}.{minor} is outside reviewed stable 2.x coverage"
        ))),
        None => Err(AdapterError::Unsupported(format!(
            "installed InfluxDB version {value} is invalid"
        ))),
    }
}
...
fn platform(context: &SystemContext, manager: Manager) -> Result<Platform, AdapterError> {
    let distribution = context.distribution.as_ref().ok_or_else(|| {
        AdapterError::Unsupported("InfluxDB packages require a detected Linux distribution".into())
    })?;
    ...
    match manager {
        Manager::Apt => {
            if !matches!(distribution.id.as_str(), "debian" | "ubuntu") {
                return Err(AdapterError::Unsupported(format!(
                    "InfluxDB stable APT coverage excludes {}",
                    distribution.id
                )));
            }
            Ok(Platform {
        ...
        Manager::Rpm => {
            ...
            {
                return Err(AdapterError::Unsupported(format!(
                    "InfluxDB stable RPM coverage is EL9 only, not {} {}",
...
fn mapped_uri(
    ...
    match (manager, provider) {
        (Manager::Apt, "nju" | "tuna") => Ok(format!("{endpoint}/{}", platform.family)),
        (Manager::Apt, "ustc") if platform.family == "debian" => Ok(format!("{endpoint}/debian")),
        (Manager::Apt, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed current InfluxDB APT repository".into(),
        )),
        (Manager::Rpm, "nju" | "tuna") if platform.architecture == "x86_64" => {
            Ok(format!("{endpoint}/yum/el9-x86_64"))
        }
        (Manager::Rpm, _) => Err(AdapterError::InvalidConfiguration(
            "selected provider has no reviewed InfluxDB RPM repository".into(),
        )),
    }
}
...
fn representative_package_name(
    ...
    match (
        ...
        (Manager::Apt, _, "stable", "amd64") => Ok("influxdb2_2.9.1-1_amd64.deb"),
        (Manager::Apt, _, "stable", "arm64") => Ok("influxdb2_2.9.1-1_arm64.deb"),
        (Manager::Rpm, "el", "9", "x86_64") => Ok("influxdb2-2.9.1.x86_64.rpm"),
        _ => Err(AdapterError::Unsupported(
            "InfluxDB representative package is unavailable for this target".into(),
        )),
    }
}
...
fn rewrite_existing(
    ...
    let source = parse_repository(manager, text, path)?.ok_or_else(|| {
        AdapterError::InvalidConfiguration("selected InfluxDB repository disappeared".into())
    })?;
    if source.channel != channel {
        return Err(AdapterError::Unsupported(
            "InfluxDB repository channel changed during planning".into(),
        ));
    }
...
fn replace_active_once(
    ...
    if ranges.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "InfluxDB repository URI in {} is missing or duplicated",
            path.display()
        )));
    }
...
fn selected_endpoint(selections: &[MirrorSelection]) -> Result<(String, String), AdapterError> {
    if selections.len() != 1
        || selections[0].tool_id != "influxdb"
        || selections[0].upstream_id != UPSTREAM
    {
        return Err(AdapterError::InvalidConfiguration(
            "InfluxDB packages require exactly one repository selection".into(),
        ));
    }
    ...
    let role_url = |role| -> Result<String, AdapterError> {
        ...
        if endpoints.len() != 1 {
            return Err(AdapterError::InvalidConfiguration(format!(
                "InfluxDB package selection needs one HTTPS {role:?} endpoint"
            )));
        }
        normalized_endpoint(&endpoints[0].url).ok_or_else(|| {
            AdapterError::InvalidConfiguration("InfluxDB package endpoint is unsafe".into())
        })
    };
    let index = role_url(EndpointRole::Index)?;
    if role_url(EndpointRole::Metadata)? != index || role_url(EndpointRole::Packages)? != index {
        return Err(AdapterError::InvalidConfiguration(
            "InfluxDB package index, metadata, and package endpoints must match".into(),
        ));
    }
    ...
    {
        return Err(AdapterError::InvalidConfiguration(
            "InfluxDB package provider and endpoint are not reviewed".into(),
        ));
    }
...
fn validate_policy(current: &CurrentConfiguration) -> Result<(), AdapterError> {
    for source in &current.sources {
        match metadata(source, "kind")? {
            "multiple-modern-repositories" => {
                return Err(AdapterError::InvalidConfiguration(
                    "multiple modern InfluxDB package repositories are active".into(),
                ));
            }
            "coverage-mismatch" => {
                return Err(AdapterError::Unsupported(
                    "InfluxDB repository distribution, release, or channel does not match the host"
                        .into(),
                ));
            }
            "keyring-missing" => {
                return Err(AdapterError::Unsupported(
                    "InfluxDB APT keyring is missing; repository security cannot be preserved"
                        .into(),
                ));
            }
            "gpgcheck-disabled" => {
                return Err(AdapterError::Unsupported(
                    "InfluxDB RPM repository has gpgcheck disabled".into(),
                ));
            }
            "gpgkey-missing" => {
                return Err(AdapterError::Unsupported(
                    "InfluxDB RPM repository has no GPG key configured".into(),
                ));
            }
            "managed-target-conflict" => {
                return Err(AdapterError::Unsupported(
                    "InfluxDB package target belongs to another MirrorSwitch format".into(),
                ));
            }
            _ => {}
        }
...
fn verify_apt(runtime: &dyn Runtime, target: &Path) -> Result<String, AdapterError> {
    ...
    command_output(runtime.run("apt-get", &update)?, "InfluxDB APT refresh")?;
    let mut query = Vec::new();
    for option in &options {
        query.extend(["-o".into(), option.clone()]);
    }
    query.extend(["policy".into(), "influxdb2".into()]);
    command_output(runtime.run("apt-cache", &query)?, "InfluxDB APT query")
}
...
fn verify_rpm(
    ...
    let repo_id = repo_id.unwrap_or("mirrorswitch-influxdb");
    ...
    command_output(runtime.run("dnf", &refresh)?, "InfluxDB RPM refresh")?;
    ...
    command_output(runtime.run("dnf", &query)?, "InfluxDB RPM query")
}
...
fn find_document<'a>(
    ...
    if matches.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "InfluxDB package state must contain one {format} document"
        )));
    }
    Ok(matches[0])
}
...
fn current_value(current: &CurrentConfiguration, kind: &str) -> Result<String, AdapterError> {
    ...
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "InfluxDB package state has ambiguous {kind}"
        )));
    }
    Ok(values[0].clone())
}
...
fn snapshot_source(kind: &str, value: &str) -> ConfiguredSource {
    ConfiguredSource {
        upstream_id: None,
        url: format!("influxdb-package-snapshot:{value}"),
        enabled: true,
        metadata: BTreeMap::from([
...
fn metadata<'a>(source: &'a ConfiguredSource, key: &str) -> Result<&'a str, AdapterError> {
    let values = source.metadata.get(key).ok_or_else(|| {
        AdapterError::InvalidConfiguration(format!(
            "InfluxDB package source is missing {key} metadata"
        ))
    })?;
    if values.len() != 1 {
        return Err(AdapterError::InvalidConfiguration(format!(
            "InfluxDB package source has ambiguous {key} metadata"
        )));
    }
    Ok(&values[0])
}
...
fn validate_path(path: &Path, kind: &str) -> Result<(), AdapterError> {
    ...
    {
        return Err(AdapterError::InvalidConfiguration(format!(
            "InfluxDB package {kind} path {} is unsafe",
            path.display()
        )));
    }
    Ok(())
...
fn utf8<'a>(path: &Path, contents: &'a [u8]) -> Result<&'a str, AdapterError> {
    std::str::from_utf8(contents).map_err(|_| {
        AdapterError::InvalidConfiguration(format!(
            "InfluxDB package configuration {} is not UTF-8",
            path.display()
        ))
    })
}
